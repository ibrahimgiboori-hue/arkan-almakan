...
export function projectCaretakerState(project, financial = {}, facts = {}) {
  ...
  const fieldComplete = project?.stage === 'closed' || progress >= 99.999;
  const outstanding =
  ...
  if (fieldComplete) return outstanding ? 'closing' : 'closed';
  if (project?.stage === 'lost' || project?.status === 'closed') return 'closed';
