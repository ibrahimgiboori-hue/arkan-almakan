...
export function buildProjectReportDraft(snapshot) {
  ...
  const titleItems = (snapshot.items || [])
    .filter((item) => String(item.kind || '') === 'title')
    .map((item) => item.description_ar)
    .filter(Boolean);
  const introParts = [
    `يعرض هذا التقرير الموقف المسجل لمشروع «${project.name_ar || project.project_no || 'المشروع'}» حتى تاريخ ${reportDate}`,
    'وقد تم تكوينه آليًا من بيانات المشروع المثبتة في النظام دون إضافة أرقام أو وقائع غير مسجلة',
  ];
  if (titleItems.length) introParts.push(`ويشمل نطاق العمل المسجل: ${titleItems.join('، ')}`);
  ...
  return {
    ...
    payload:{
      ...
      _auto_report:{
        version:'project-intelligence-v1',
        generated_at:new Date().toISOString(),
        project_id:project.id,
        project_no:project.project_no,
        source_tables:[
          'projects','project_items','v_item_progress','item_measurements','progress_claims',
