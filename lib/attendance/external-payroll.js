import { ARABIC_GREGORIAN_LATIN_LOCALE, LATIN_NUMBER_LOCALE } from '@/lib/latin-digits';
...
export const NATIONALITY_CATEGORY_LABEL = Object.fromEntries(NATIONALITY_CATEGORIES);

export const SOCIAL_INSURANCE_SCHEMES = [
  ['saudi_legacy','النظام القائم'],
  ['saudi_new','النظام الجديد'],
  ['manual','نسبة مخصصة'],
];

export const SOCIAL_INSURANCE_SCHEME_LABEL = Object.fromEntries(SOCIAL_INSURANCE_SCHEMES);
export const GOSI_CONTRIBUTORY_WAGE_MAX = 45000;
export const SANED_EMPLOYEE_RATE = 0.75;
...
export function payrollMonthLabel(periodFrom, locale=ARABIC_GREGORIAN_LATIN_LOCALE) {
...
export function formatMoney(value) {
  ...
  return new Intl.NumberFormat(LATIN_NUMBER_LOCALE,{minimumFractionDigits:2,maximumFractionDigits:2}).format(n);
}
...
export function newSaudiPensionEmployeeRate(periodFrom) {
  const key = dateOnly(periodFrom) || new Date().toISOString().slice(0,10);
  if (key < '2025-07-03') return 9;
  if (key < '2026-07-03') return 9.5;
  if (key < '2027-07-03') return 10;
  if (key < '2028-07-03') return 10.5;
  return 11;
}

export function resolveEmployeeSocialInsuranceRate({ active=false, nationalityCategory, scheme, periodFrom, override } = {}) {
  if (!active) return {ready:true,rate:0,scheme:null};
  if (!nationalityCategory) return {ready:false,rate:null,reason:'حدد الجنسية.'};

  if (nationalityCategory === 'non_saudi') {
    return {ready:true,rate:0,scheme:'non_saudi'};
  }

  if (nationalityCategory === 'gcc') {
    const custom = numberOrNull(override);
    if (custom == null || custom < 0 || custom > 100) {
      return {ready:false,rate:null,reason:'أدخل نسبة حصة الموظف الخليجي في التأمينات.'};
    }
    return {ready:true,rate:custom,scheme:'manual'};
  }

  if (nationalityCategory === 'saudi') {
    if (scheme === 'saudi_new') {
      return {ready:true,rate:newSaudiPensionEmployeeRate(periodFrom)+SANED_EMPLOYEE_RATE,scheme};
    }
    if (scheme === 'saudi_legacy') {
      return {ready:true,rate:9+SANED_EMPLOYEE_RATE,scheme};
    }
    return {ready:false,rate:null,reason:'حدد نظام اشتراك الموظف السعودي.'};
  }

  return {ready:false,rate:null,reason:'الجنسية غير معروفة.'};
}

export function salaryBreakdown({ line = {}, profile = {}, periodFrom } = {}) {
  const basic = numberOrNull(line?.basic_salary);
  if (basic == null || basic < 0) {
    return {ready:false,reason:'أدخل الراتب الأساسي.'};
  }

  const nationalityCategory = profile?.nationality_category || '';
  if (!nationalityCategory) {
    return {ready:false,reason:'حدد الجنسية.'};
  }

  const housing = Math.max(0,numberOrZero(line?.housing_allowance));
  const transport = Math.max(0,numberOrZero(line?.transport_allowance));
  const other = Math.max(0,numberOrZero(line?.other_allowances));
  const insuranceActive = profile?.social_insurance_active === true;
  const insurance = resolveEmployeeSocialInsuranceRate({
    active:insuranceActive,
    nationalityCategory,
    scheme:profile?.social_insurance_scheme || '',
    periodFrom,
    override:line?.gosi_employee_rate_override,
  });
  if (!insurance.ready) return insurance;

  const grossSalary = round2(basic + housing + transport + other);
  const contributoryWage = round2(Math.min(GOSI_CONTRIBUTORY_WAGE_MAX,basic + housing));
  const gosiEmployeeRate = insuranceActive ? Number(insurance.rate || 0) : 0;
  const gosiEmployeeDeduction = insuranceActive ? round2(contributoryWage * gosiEmployeeRate / 100) : 0;
  const referenceNetSalary = round2(grossSalary - gosiEmployeeDeduction);
  ...
  return {
    ...
    grossSalary,
    nationalityCategory,
    socialInsuranceActive:insuranceActive,
    contributoryWage,
    socialInsuranceScheme:insurance.scheme,
    gosiEmployeeRate,
...
export function calculateExternalPayroll({
  ...
  const salary = salaryBreakdown({line,profile,periodFrom});
  if (!salary.ready) return {ready:false,reason:salary.reason};

  const referenceNet = salary.referenceNetSalary;
  ...
  for (const day of days) {
    ...
    if (delta > 0) rawExtraMinutes += delta;
    if (delta < 0) rawShortMinutes += Math.abs(delta);
  }

  const includeOvertime = batch?.include_overtime !== false;
  const includeTimeShortage = batch?.include_time_shortage !== false;
  const extraMinutes = includeOvertime ? rawExtraMinutes : 0;
  const shortMinutes = includeTimeShortage ? rawShortMinutes : 0;
  const rawNetMinutes = rawExtraMinutes - rawShortMinutes;
  ...
  const timeAmount = round2((netMinutes/60)*hourValue);
  ...
  const snapshot = {
    ...
    employee: {
      ...
      nationality_category: salary.nationalityCategory,
      social_insurance_active: salary.socialInsuranceActive,
      show_job_title: !!line?.show_job_title,
      show_identity: !!line?.show_identity,
    },
    salary: {
      basic_salary: salary.basicSalary,
      housing_allowance: salary.housingAllowance,
      transport_allowance: salary.transportAllowance,
      other_allowances: salary.otherAllowances,
      gross_salary: salary.grossSalary,
      nationality_category: salary.nationalityCategory,
      social_insurance_active: salary.socialInsuranceActive,
      social_insurance_scheme: salary.socialInsuranceScheme,
      contributory_wage: salary.contributoryWage,
      gosi_employee_rate: salary.gosiEmployeeRate,
      gosi_employee_deduction: salary.gosiEmployeeDeduction,
      reference_net_salary: salary.referenceNetSalary,
    },
    attendance: {
      ...
      raw_extra_minutes: rawExtraMinutes,
      raw_short_minutes: rawShortMinutes,
      raw_net_minutes: rawNetMinutes,
    ...
    calculation: {
      ...
      include_time_shortage: includeTimeShortage,
  ...
  return {
    ...
    nationalityCategory:salary.nationalityCategory,
    socialInsuranceActive:salary.socialInsuranceActive,
    contributoryWage:salary.contributoryWage,
    socialInsuranceScheme:salary.socialInsuranceScheme,
    gosiEmployeeRate:salary.gosiEmployeeRate,
    gosiEmployeeDeduction:salary.gosiEmployeeDeduction,
    referenceNetSalary:salary.referenceNetSalary,
    divisorDays,
    dayHours,
    dayValue,
    hourValue,
