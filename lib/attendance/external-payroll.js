...
export const SOCIAL_INSURANCE_SCHEMES = [
  ['saudi_legacy','سعودي — النظام القائم'],
  ['saudi_new','سعودي — النظام الجديد'],
  ['non_saudi','غير سعودي'],
  ['manual','نسبة مخصصة / خليجي'],
];
...
export function resolveEmployeeSocialInsuranceRate({ scheme, periodFrom, override } = {}) {
  const custom = numberOrNull(override);
  if (!scheme) return { ready:false, rate:null, reason:'حدد فئة التأمينات للموظف.' };
  if (scheme === 'manual') {
    if (custom == null || custom < 0 || custom > 100) {
      return { ready:false, rate:null, reason:'أدخل نسبة حصة الموظف في التأمينات للحالة المخصصة.' };
    }
    return { ready:true, rate:custom };
  }
  if (scheme === 'non_saudi') return { ready:true, rate:0 };
  if (scheme === 'saudi_legacy') return { ready:true, rate:9 + SANED_EMPLOYEE_RATE };
  if (scheme === 'saudi_new') {
    return { ready:true, rate:newSaudiPensionEmployeeRate(periodFrom) + SANED_EMPLOYEE_RATE };
  }
  return { ready:false, rate:null, reason:'فئة التأمينات غير معروفة.' };
}
...
export function salaryBreakdown({ line = {}, profile = {}, periodFrom } = {}) {
  ...
  const scheme = profile?.social_insurance_scheme || '';
  ...
  const gosiEmployeeRate = Number(insurance.rate || 0);
  const gosiEmployeeDeduction = round2(contributoryWage * gosiEmployeeRate / 100);
