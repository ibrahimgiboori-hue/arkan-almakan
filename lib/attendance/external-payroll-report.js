...
function buildRow(line,profile,attendanceDays){
  ...
  const visibleEarnings=roundMoney(basicSalary+housingAllowance+transportAllowance+otherAllowances+overtimeAddition+otherAdditions);
  const visibleDeductions=roundMoney(absenceDeduction+timeDeduction+subscriptionDeduction+otherDeductions);
...
export function buildExternalPayrollReport({batch={},attendanceImport={},lines=[],profiles=[],days=[]}={}){
  ...
  const totals=Object.freeze({
    ...
    additions:roundMoney(totalOvertime+totalOtherAdditions),
    ...
    deductions:roundMoney(totalAbsenceDeduction+totalTimeDeduction+totalSubscriptionDeduction+totalOtherDeductions),
