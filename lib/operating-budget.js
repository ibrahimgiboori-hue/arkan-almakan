export const OPERATING_BUDGET = Object.freeze({
  ...
  calculationLabels: Object.freeze({
    ...
    tiered: 'شرائح',
    percentage_of_base: 'نسبة من أساس الاحتساب',
  ...
  componentModeLabels: Object.freeze({
    fixed: 'قيمة ثابتة',
    input_amount: 'قيمة المدخل كما هي',
    percentage_of_input: 'نسبة من أساس الاحتساب',
...
const SINGLE_INPUT_COMPONENT_MODES = new Set(['input_amount', 'percentage_of_input', 'per_unit', 'input_times_constant']);
...
export function budgetInputFields(calculationType, rateParams = {}) {
  switch (calculationType) {
    ...
    case 'percentage_of_base':
      return [
        { key: 'base_amount', label: 'أساس الاحتساب', ...MONEY_FIELD },
...
export function budgetRateFields(calculationType) {
  switch (calculationType) {
    ...
    case 'percentage_of_base':
      return [
        { key: 'base_amount', label: 'أساس الاحتساب الافتراضي', ...MONEY_FIELD },
        { key: 'percentage', label: 'النسبة %', ...NUMBER_FIELD },
      ];
    case 'fixed_amount':
...
export function budgetComponentInputOptions(calculationType, inputSchema = [], rateParams = {}) {
  const explicit = normalizeBudgetInputSchema(inputSchema);
  if (explicit.length) return explicit;
  return budgetInputFields(calculationType, rateParams).filter((field) => !['percentage', 'unit_price'].includes(field.key));
}

export function budgetComponentNeedsSingleInput(mode) {
  return SINGLE_INPUT_COMPONENT_MODES.has(mode);
}

export function budgetValidateComponentInputs(calculationType, inputSchema = [], components = [], rateParams = {}) {
  const options = budgetComponentInputOptions(calculationType, inputSchema, rateParams);
  const allowed = new Set(options.map((field) => field.key));
  const errors = [];

  for (const [index, component] of (Array.isArray(components) ? components : []).entries()) {
    const label = component?.label || `المكون ${index + 1}`;
    if (SINGLE_INPUT_COMPONENT_MODES.has(component?.mode)) {
      if (!component?.input_key) {
        errors.push(`${label}: اختر أساس الاحتساب.`);
      } else if (!allowed.has(component.input_key)) {
        errors.push(`${label}: أساس الاحتساب غير موجود ضمن مدخلات الحساب المعتمدة.`);
      }
    }
    if (component?.mode === 'multiply_inputs') {
      if (!component?.left_input_key || !component?.right_input_key) {
        errors.push(`${label}: اختر المدخلين المطلوبين للحساب.`);
      } else {
        if (!allowed.has(component.left_input_key)) errors.push(`${label}: المدخل الأول غير معتمد.`);
        if (!allowed.has(component.right_input_key)) errors.push(`${label}: المدخل الثاني غير معتمد.`);
      }
    }
  }
...
export function budgetWorkGuidance(calculationType, options = {}) {
  const { nodeType = 'item', mode = 'catalog' } = options;
  if (mode === 'statement') {
    const fields = budgetInputFields(calculationType, options.rateParams || {});
    return {
      context: 'كشف الشهر › تعديل التفصيل',
      summary: fields.length ? 'عدّل المدخلات التي تغيّر التقدير، أو ثبّت القيمة الفعلية من الفاتورة.' : 'ثبّت القيمة الفعلية عندما تصبح الفاتورة أو القيمة النهائية معروفة.',
      steps: fields.length ? [...fields.map((field) => field.label), 'القيمة الفعلية عند توفرها'] : ['القيمة الفعلية عند توفرها'],
    };
  }

  if (nodeType === 'group') {
    return {
      context: 'كتالوج التشغيل › تصنيف',
      summary: 'التصنيف ينظم التفاصيل فقط ولا يحمل مبلغًا مستقلًا.',
      steps: ['مكان التصنيف', 'الاسم', 'الحفظ'],
    };
  }

  const byType = {
    fixed_amount: {
      summary: 'عرّف المبلغ الثابت أولًا، ثم حدّد متى يبدأ ومتى يحين استحقاقه.',
      steps: ['المبلغ', 'سريان القاعدة', 'توقيت الاستحقاق'],
    },
    variable_monthly: {
      summary: 'ابدأ بمتوسط شهري واقعي، ثم ثبّت بداية السريان واليوم المعتاد لنزول الفاتورة.',
      steps: ['المتوسط التقريبي', 'بداية السريان', 'موعد نزول الفاتورة'],
    },
    quantity_x_unit_price: {
      summary: 'القيمة تأتي من الكمية وسعر الوحدة؛ الجدولة تحدد متى يظهر الالتزام.',
      steps: ['الكمية', 'سعر الوحدة', 'الجدولة'],
    },
    percentage_of_base: {
      summary: 'لا توجد نسبة بلا أساس احتساب صريح. عرّف الأساس والنسبة ثم الجدولة.',
      steps: ['أساس الاحتساب', 'النسبة', 'الجدولة'],
    },
    tiered: {
      summary: 'عرّف حدود الشرائح وقيمة كل شريحة قبل ربطها بتوقيت الاستحقاق.',
      steps: ['حدود الشرائح', 'قيم الشرائح', 'الجدولة'],
    },
    employee_based_contribution: {
      summary: 'عرّف مدخل الحساب مرة واحدة، ثم اربط كل مكوّن بأساس احتسابه واكتب نسبته.',
      steps: ['مدخل الحساب', 'أساس احتساب المكوّن', 'النسبة', 'الجدولة'],
    },
    subscription_plus_usage: {
      summary: 'افصل الاشتراك الأساسي عن الاستخدام، واربط كل جزء بمدخله عند الحاجة.',
      steps: ['مدخلات الحساب', 'المكونات', 'الجدولة'],
    },
    composite_formula: {
      summary: 'عرّف المدخلات أولًا ثم كوّن الحساب من مكونات آمنة مرتبطة بها صراحةً.',
      steps: ['المدخلات', 'المكونات', 'الجدولة'],
    },
    manual_actual: {
      summary: 'هذا البند يعتمد على قيمة تدخل يدويًا؛ ثبّت السريان والجدولة بوضوح.',
      steps: ['القيمة', 'السريان', 'الجدولة'],
    },
    external_forecast_actual: {
      summary: 'القيمة تأتي من المصدر النظامي؛ المطلوب هنا تنظيم السريان والتوقيت فقط.',
      steps: ['السريان', 'الجدولة'],
    },
  };
  return {
    context: 'كتالوج التشغيل › إعداد عنصر',
    ...(byType[calculationType] || { summary: 'أكمل قاعدة الحساب ثم الجدولة.', steps: ['قاعدة الحساب', 'الجدولة'] }),
  };
}
