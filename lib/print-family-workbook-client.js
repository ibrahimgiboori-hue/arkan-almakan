...
function pruneVoucherCellNoise(cells) {
  ...
  for (const cell of cells) {
    ...
    if (borders.top?.intentional) edges.push({cell,side:'top',axis:'h',line:cell.row,start:cell.col,end:endCol,border:borders.top});
    if (borders.bottom?.intentional) edges.push({cell,side:'bottom',axis:'h',line:endRow + 1,start:cell.col,end:endCol,border:borders.bottom});
    if (borders.left?.intentional) edges.push({cell,side:'left',axis:'v',line:cell.col,start:cell.row,end:endRow,border:borders.left});
    if (borders.right?.intentional) edges.push({cell,side:'right',axis:'v',line:endCol + 1,start:cell.row,end:endRow,border:borders.right});
  }
  ...
  function connected(edge) {
    if (strong(edge)) return true;
    return edges.some((other) => {
      if (other === edge || other.axis !== edge.axis || other.line !== edge.line) return false;
      if (String(other.border?.color || '') !== String(edge.border?.color || '')) return false;
      // A real document rule is a continuous run. A one-cell tick has no neighbour.
  ...
  for (const edge of edges) {
    if (connected(edge)) continue;
    if (edge.cell?.style?.borders) edge.cell.style.borders[edge.side] = null;
  }
  return cells;
}

function extractModelSheet(sheet, XLSX, name, pageConfig, rawStyleMap = new Map()) {
  ...
  for (let r = used.s.r; r <= used.e.r; r += 1) {
    for (let c = used.s.c; c <= used.e.c; c += 1) {
      ...
      if (!text.trim() && !hasRenderableStyle(rawStyle)) continue;
...
function buildUiSchema(workbook, XLSX, familyId, modelSheets, rawStyleMaps = new Map()) {
  ...
  const models = modelSheets.map((name) => ({
    ...extractModelSheet(workbook.Sheets?.[name], XLSX, name, pageConfig, rawStyleMaps.get(name) || new Map()),
