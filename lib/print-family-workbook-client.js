...
function extractModelSheet(sheet, XLSX, name) {
  const cells = [];
  const tokenCells = [];
  const staticCells = [];
  ...
  if (!sheet?.['!ref']) {
    return { name, tokens:[], tokenLabels:{}, cells:[], columns:[], rows:[], hasQty:false, hasVat:false };
  }
...
function buildUiSchema(workbook, XLSX, familyId, modelSheets) {
  ...
  const models = modelSheets.map((name) => extractModelSheet(workbook.Sheets?.[name], XLSX, name));
  return {
    schema:'arkan-workbook-ui-v2',
    familyId,
    variables,
    flowRules,
    visibility,
