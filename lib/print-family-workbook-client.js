...
function normalizeBorderSide(side) {
  ...
  const widthMm = style === 'thick' ? 0.55 : (style === 'medium' || style === 'mediumDashed' || style === 'mediumDashDot' || style === 'mediumDashDotDot') ? 0.38 : 0.2;
  const lineStyle = /dash/i.test(style) ? 'dashed' : /dot/i.test(style) ? 'dotted' : 'solid';
  return { widthMm, lineStyle, color:normalizeStyleColor(side?.color) || '#8f8f8f' };
}
...
function hasRenderableStyle(style) {
  ...
  return Boolean(
    normalized.fillColor || normalized.fontColor || normalized.bold || normalized.italic || normalized.fontSizePt
    || normalized.borders.top || normalized.borders.right || normalized.borders.bottom || normalized.borders.left
  );
}

function extractModelSheet(sheet, XLSX, name, pageConfig) {
  ...
  for (let r = used.s.r; r <= used.e.r; r += 1) {
    for (let c = used.s.c; c <= used.e.c; c += 1) {
      ...
      if (!text.trim() && !hasRenderableStyle(cell.s)) continue;
...
function buildUiSchema(workbook, XLSX, familyId, modelSheets) {
  ...
  const models = modelSheets.map((name) => ({
    ...extractModelSheet(workbook.Sheets?.[name], XLSX, name, pageConfig),
