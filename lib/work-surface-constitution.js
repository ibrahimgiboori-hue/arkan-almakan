...
export const WORK_SURFACE_POLICY = Object.freeze({
  ...
  // الجسد الجديد يحمل الأعضاء ولا يعيد بناءها. الصفحة الحالية تبقى صاحبة
  // وظيفتها وبياناتها، بينما الجسد فقط يحدد المساحة والتنقل وإيقاع سطح العمل.
  organPreservationPolicy: 'route-content-preserved-in-place',
  routeMountPolicy: 'single-route-content-host-no-cloning',
  migrationPolicy: 'move-shell-responsibility-before-retiring-page-chrome',
  navigationVisibilityPolicy: 'visible-by-default-user-hideable',
  navigationHierarchyPolicy: 'progressive-accordion-current-branch-and-siblings-visible',
  navigationSpacePolicy: 'reserve-space-on-desktop-overlay-on-touch',
  bodyChromePolicy: 'work-first-no-global-status-card-wall',
  legacyBodyPolicy: 'legacy-shells-semantic-only-until-safe-retirement',
  ...
  forbidRouteContentDuplication: true,
  forbidBodyOwnedBusinessLogic: true,
  forbidBodyFeatureDeletionDuringMigration: true,
});
...
export function surfaceDataAttributes(surface) {
  return {
    ...
    'data-body-shell-policy':WORK_SURFACE_POLICY.bodyShell,
    'data-organ-preservation-policy':WORK_SURFACE_POLICY.organPreservationPolicy,
  };
}
