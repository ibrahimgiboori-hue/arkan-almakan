...
export const AREAS = Object.freeze([
  {
    ...
    ...MODULES.home,
    items: [
      { href: '/dashboard', label: 'مركز العمل' },
      { href: '/dashboard/my-work', label: 'أعمالي' },
      { href: '/dashboard/my-work/approvals', label: 'اعتماداتي', capabilities: ['system.approvals.view'] },
      { href: '/dashboard/today', label: 'اليوم القديم', hidden: true, legacy: true },
    ],
  },
  {
    key: 'projects',
    ...
    items: [
      ...
      // مسارات التشغيل العامة القديمة تبقى توافقية فقط، ولا تظهر في الملاحة لأن بديلها داخل المشروع نفسه.
  ...
  },
  {
    ...
    ...MODULES.workforce,
    items: [
      // الإنشاء ليس أداة ملاحة مستقلة: إضافة الموظف تتم من سجل الموظفين.
      { href: '/dashboard/employees', label: 'الموظفون' },
      { href: '/dashboard/attendance', label: 'الحضور والانصراف', capabilities: ['hr.attendance.view'] },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/dashboard/operating-budget', label: 'ميزانية وتشغيل الشركة', capabilities: ['finance.operating_budget.view'] },
      { href: '/dashboard/approvals', label: 'الاعتمادات' },
    ],
  },
  {
    key: 'documents',
...
// ترتيب أدوات المشروع جزء من الدستور: اليوم أولاً، ثم ما يُسجَّل ويُدار، ثم ما يُقرأ ويتابع.
// لا يُعاد تعريف أداة عامة هنا إذا كان لها مدخل واحد قائم على مستوى بوابة المشاريع.
export const PROJECT_NAV_GROUPS = Object.freeze([
  ...
  {
    key: 'entry',
    label: 'التسجيل والإدارة',
    items: Object.freeze([
      { key: 'labor', label: 'العمالة', suffix: '/operations/labor' },
...
const PROJECT_VIEW_KEYS = new Set(['anatomy','overview','scope','progress','claims','guarantees','docs','settings']);

export function normalizeProjectView(value) {
  if (value === 'exec') return 'scope';
  return PROJECT_VIEW_KEYS.has(value) ? value : 'anatomy';
}

export function projectNavigationHref(projectId, item, context = {}) {
  if (item?.href) return item.href;
  const base = `/dashboard/projects/${projectId}`;
  const path = item?.suffix !== undefined ? `${base}${item.suffix}` : base;
  const params = new URLSearchParams();
  if (item?.suffix === undefined) {
    const view = normalizeProjectView(item?.view);
    if (view !== 'anatomy') params.set('view', view);
  }
  if (context?.care) params.set('care', String(context.care));
  if (context?.region) params.set('region', String(context.region));
