...
export const quoteEditorSupabaseRepository=Object.freeze({
  ...
  async loadPrintFamilySchema(familyId){
    const {data,error}=await supabase.from('print_family_workbooks')
      .select('ui_schema,model_sheets,version')
      .eq('family_id',familyId)
      .maybeSingle();
    fail(error,'تعذر تحميل مخطط واجهة عائلة المطبوعات.');
    return data || null;
  },

  async loadPrintAssets(){
    const {data,error}=await supabase.from('app_settings')
      .select('stamp_image_path,signature_image_path,letterhead_image_path,header_image_path,watermark_image_path,footer_image_path,header_height_mm,footer_height_mm,print_white_veil_opacity,print_side_margin_preset')
      .eq('id',1)
      .maybeSingle();
    fail(error,'تعذر تحميل أصول الطباعة.');
    const publicUrl=(path)=>path ? supabase.storage.from('brand').getPublicUrl(path).data.publicUrl : '';
    return {
      stamp:publicUrl(data?.stamp_image_path),
      signature:publicUrl(data?.signature_image_path),
      letterhead:publicUrl(data?.letterhead_image_path),
      header:publicUrl(data?.header_image_path),
      watermark:publicUrl(data?.watermark_image_path),
      footer:publicUrl(data?.footer_image_path),
      headerHeightMm:Number(data?.header_height_mm || 0),
      footerHeightMm:Number(data?.footer_height_mm || 0),
      whiteVeilOpacity:Number(data?.print_white_veil_opacity ?? 0.82),
      sideMarginPreset:['small','medium','large'].includes(data?.print_side_margin_preset)
        ? data.print_side_margin_preset
        : 'small',
    };
  },
