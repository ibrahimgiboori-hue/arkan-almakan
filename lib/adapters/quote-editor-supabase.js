...
export const quoteEditorSupabaseRepository=Object.freeze({
  ...
  async loadPrintAssets(){
    const {data,error}=await supabase.from('app_settings')
      .select('stamp_image_path,signature_image_path,letterhead_image_path,stationery_analysis,print_white_veil_opacity,print_side_margin_preset')
      .eq('id',1)
      .maybeSingle();
    fail(error,'تعذر تحميل أصول الطباعة.');
    const publicUrl=(path)=>path ? supabase.storage.from('brand').getPublicUrl(path).data.publicUrl : '';
    const analysis=data?.stationery_analysis || {};
    return {
      ...
      stationeryAnalysis:analysis,
      headerHeightMm:Number(analysis?.headerHeightMm || 0),
      footerHeightMm:Number(analysis?.footerHeightMm || 0),
