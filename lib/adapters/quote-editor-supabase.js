...
export const quoteEditorSupabaseRepository=Object.freeze({
  ...
  async loadPrintAssets(){
    const {data,error}=await supabase.from('app_settings')
      .select('stamp_image_path,signature_image_path')
      .eq('id',1)
      .maybeSingle();
    fail(error,'تعذر تحميل الختم والتوقيع.');
    const stampPath=data?.stamp_image_path || '';
    const signaturePath=data?.signature_image_path || '';
    return {
      stamp:stampPath ? supabase.storage.from('brand').getPublicUrl(stampPath).data.publicUrl : '',
      signature:signaturePath ? supabase.storage.from('brand').getPublicUrl(signaturePath).data.publicUrl : '',
    };
  },
