import { legacyTenantContext, resolveTenantContext, tenantWithModules } from '../core/tenant-context';
import {
...
async function loadTenantSnapshot(client, session) {
  ...
  let tenant = resolveTenantContext({
    memberships,
  ...
  if (tenant.mode === 'multi_tenant' && tenant.activeOrganization?.id) {
    ...
    const modulesQ = await client
      .from('organization_modules')
      .select('module_key,status,settings')
    ...
    tenant = tenantWithModules(tenant,modulesQ.data || []);
  } else {
    clearActiveOrganizationId({ announce:false });
  }
