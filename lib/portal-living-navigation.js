...
function sourceGroupsForPortal(portalKey) {
  const groups = SHELL_PORTAL_GROUPS[portalKey] || [];
  if (portalKey !== 'projects') return groups;
  // سجل المشاريع والعروض استُخدما في الشرح القديم كمجموعات، لكن في التشريح الحالي
  // يملكان حاضنات دخول حقيقية؛ نترك فقط المجموعات غير الحاضنة مثل الأطراف.
  return groups.filter((group) => !['projects','commercial'].includes(group.key));
}
...
export function livingPortalGroups(portalKey, session) {
  ...
  const groups = sourceGroupsForPortal(portalKey).map((group) => ({
    ...group,
    items:(group.hrefs || []).map((href) => byHref.get(href)).filter(Boolean),
  })).filter((group) => group.items.length > 0);

  const guardianHrefs = portalKey === 'projects'
    ? new Set(PROJECT_GUARDIANS.map((guardian) => guardian.href.split('?')[0]))
    : new Set();
...
// المشاريع لديها حاضنات حياة حقيقية، ولذلك تقدمها كعقد دخول بدل سجل ثابت،
...
export function portalEntryNodes(portalKey, session) {
  ...
  const guardians = PROJECT_GUARDIANS
    .filter((guardian) => guardian.entityKind === 'project' || toolHrefs.has(guardian.href.split('?')[0]))
    .map((guardian) => ({
  ...
  const auxiliaryGroups = groups.map((group) => ({
    key:`group:${group.key}`,
    nodeKind:'group',
    label:group.label,
