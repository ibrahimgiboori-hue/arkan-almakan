...
export function summarizeWorkdaysByLaborClass(rows = [], fallbackByLaborer = {}) {
  const summary = { technician:0, worker:0, foreman:0, other:0, total:0 };
  for (const row of rows || []) {
    const factor = attendanceFactor(row.status);
    const laborClass = row.labor_class || row.laborClass || fallbackByLaborer[row.laborer_id] || '';
    if (laborClass === 'technician') summary.technician += factor;
    else if (laborClass === 'worker') summary.worker += factor;
    else if (laborClass === 'foreman') summary.foreman += factor;
    else summary.other += factor;
