...
export function createProjectClaimsService(repository=projectClaimsSupabaseRepository){
  return Object.freeze({
    ...
    async decideApproval({claimId=null,workflowId,decision,note}){
      const payload=projectClaimApprovalDecision({workflowId,decision,note});
      const result=await repository.decideApproval(payload);
      if(result===null||result===undefined)throw new Error('لم يعد الخادم بإثبات قرار الاعتماد.');
      if(!claimId)return Object.freeze({result});
      const context=await repository.loadJourneyContext(claimId);
      if(context?.approval?.workflow?.id!==workflowId)throw new Error('تعذر إثبات قرار الاعتماد على رحلة المستخلص نفسها.');
