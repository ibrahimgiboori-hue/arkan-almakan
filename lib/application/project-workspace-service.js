...
function settledError(result,label){
  return result?.status==='rejected'?`${label}: ${result.reason?.message||result.reason}`:null;
}
...
export function createProjectWorkspaceService(repository=projectWorkspaceSupabaseRepository){
  return Object.freeze({
    async loadWorkspace({projectId}){
      ...
      const source=await repository.loadSource(projectId);
      const [financialResult,setupResult]=await Promise.allSettled([
        repository.loadFinancials(projectId),
        repository.loadSetupQueue(projectId),
      ]);
      const financialSource=settledValue(financialResult,{financials:null,totals:null});
      const setupRows=settledValue(setupResult,[]);
      const readErrors=[
        settledError(financialResult,'الموقف المالي'),
        settledError(setupResult,'رحلة اعتماد التأسيس'),
      ].filter(Boolean);
      return Object.freeze({
