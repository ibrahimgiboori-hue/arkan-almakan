...
function requireRepository(repository){
  ...
  for(const method of [
    'getImport','loadProcessingDays','startReview','submitJustification','decideJustification','recalculateImport',
  ]){
...
export function createExternalAttendanceReviewService(repository){
  ...
  async function list(limit=40){
    if(typeof repo.listImports!=='function')throw new Error('External attendance repository is missing listImports.');
