...
export function resolveOfficeMetadataFieldSpan(field = {}) {
  if (field.metadataSpan != null) return normalizeOfficeFieldSpan(field.metadataSpan);
  const key = compact(field.rawKey || field.key);
  const valueLength = String(field.rawValue ?? field.value ?? '').trim().length;

  if (['reportsubject','subject','lettertitle','topic'].includes(key)) return 48;
  if (['projectnametext','project','projectname','site','location'].includes(key)) return 36;
  if (['reportdate'].includes(key)) return 12;
  if (['reference','docnumber','issuedat','createdat','date'].includes(key)) return 24;
  if (key.includes('recipient') || key.includes('prepared') || key.includes('owner')) return 24;
  if (valueLength > 70) return 48;
  if (valueLength > 34) return 36;

  const declared = Number(field.span);
  if (Number.isFinite(declared) && declared > 0) {
    if (declared <= 12) return 12;
    if (declared <= 24) return 24;
    if (declared <= 36) return 36;
    return 48;
  }
  return 24;
}
