...
import { applyTenantModuleEntitlements } from './tenant-context.js';
...
export function composeDashboardSession(snapshot = {}) {
  ...
  const tenant = snapshot.tenant || Object.freeze({ mode:'legacy' });
  if (tenant.mode === 'error') {
    return Object.freeze({ kind:'denied', reason:'tenant_lookup_failed' });
  }
  if (tenant.mode === 'unassigned') {
    return Object.freeze({ kind:'denied', reason:'organization_unassigned' });
  }
  ...
  const { capabilities, capabilityKeys, access:rawAccess } = buildDashboardAccess({
    capabilities:rawCapabilities,
    isPrimaryUser,
    isSystemAdmin:Boolean(userRow.is_system_admin),
  });
  const access = applyTenantModuleEntitlements(rawAccess,tenant);
