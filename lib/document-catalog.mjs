...
const GROUPS = [
  ...
  ['projects_operations', [
    ...
    ['CONTRACTOR_DAILY_TIMESHEET', 'نموذج حضور عمال مقاول — فارغ', 'contractor_timesheet'],
  ]],
  ['procurement_assets', [
...
const PROFILE_RELATIONS = Object.freeze({
  ...
  contractor_timesheet: ['project', 'party', 'general'],
...
function tableFor(profile) {
  if (profile === 'contractor_timesheet') return {
    id: 'contractor_timesheet_lines', kind: 'table', style: 'strict', title: 'بيان العمالة والحضور', columns: [
      f('worker_name', 'اسم العامل', 'text', 20, { required: true }),
      f('labor_class', 'التصنيف', 'text', 7),
      f('trade', 'المهنة', 'text', 9),
      f('attendance', 'الحضور', 'text', 6),
      f('supervisor_notes', 'ملاحظات المشرف', 'text', 6),
    ],
  };
  if (profile === 'meeting_record') return {
...
export const DOCUMENT_CATALOG = Object.freeze(GROUPS.flatMap(([category, entries], groupIndex) => {
  ...
  return entries.map(([slug, nameAr, profile], itemIndex) => {
    ...
    return Object.freeze({
      ...
      profile,
      relationScope,
      descriptionAr: `${nameAr.startsWith('نموذج') ? '' : 'نموذج '}${nameAr} وفق دورة المسودة والمراجعة والاعتماد والأرشفة.`,
