...
export function buildQuotationApprovalParties(quotation, tr = (ar) => ar, { includeArkan = true, formatDate = null } = {}) {
  const q = quotation || {};
  ...
  const arkanFields = [
    ...
    { label: tr('المنصب / الصفة', 'Position / Capacity'), value: q.arkan_signatory_title || '' },
  ];
  arkanFields.push(
    { label: tr('التوقيع', 'Signature') },
    { label: tr('التاريخ', 'Date'), value: q.quote_date && typeof formatDate === 'function' ? formatDate(q.quote_date) : '' },
  );
