// DNA الفرع الحي الواحد — الجد يقود، الابن يعكس، والحفيد يخدم الأداة الحالية.
// عند عبور عتبة العمل يملك الإجراء/المعاملة المختارة المسرح وحدها؛
// قائمة الحفيد تختفي، ثم تعود عند استدعائها كرف للمعاملات الموجودة فقط.

export const NAVIGATION_MIRROR_EVENT = 'arkan:navigation-mirror-context';
export const GRANDCHILD_NAVIGATION_EVENT = 'arkan:grandchild-navigation-context';
export const RAPID_SEMANTIC_BACK_WINDOW_MS = 5000;

export const LIVING_BRANCH_POLICY = Object.freeze({
  id:'single-living-branch-v4',
  ...
  reopenedWorkNavigationPersistsUntilUserChoosesAnotherWorkSubjectOrDismisses:true,
  ...
  rapidDoubleBackWindowMs:RAPID_SEMANTIC_BACK_WINDOW_MS,
  rapidDoubleBackMeaning:'return-to-employee-desktop',
  rapidDoubleBackClosesNavigation:true,
  nonBackNavigationResetsRapidBackSequence:true,
  ...
  selectedGrandchildOwnsTheStage:true,
});
...
export function publishGrandchildNavigationContext(detail = {}) {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent(GRANDCHILD_NAVIGATION_EVENT, { detail }));
}
