...
export function canSeeArea(areaKey, access = {}) {
  if (access.fullAdmin && !access.moduleEntitlementsApplied) return true;
 ...
 */
function tenantModuleForCapability(capabilityKey = '') {
  if (capabilityKey.startsWith('projects.')) return 'projects';
  if (capabilityKey.startsWith('hr.')) return 'hr';
  if (capabilityKey.startsWith('finance.')) return 'finance_ops';
  if (capabilityKey.startsWith('documents.')) return 'documents';
  if (capabilityKey.startsWith('system.')) return 'core';
  return null;
}
...
export function canUseCapability(session, capabilityKey, scopeType = 'all', scopeKey = null) {
  ...
  if (session.tenant?.mode === 'multi_tenant') {
    const requiredModule = tenantModuleForCapability(capabilityKey);
    const moduleKeys = session.tenant.moduleKeys instanceof Set
      ? session.tenant.moduleKeys
      : new Set((session.tenant.modules || []).map((module) => module.key));
    if (requiredModule && (!moduleKeys.has('core') || !moduleKeys.has(requiredModule))) return false;
  }
...
// فتح المشروع يعني دخول المشروع نفسه، لا تشغيل أول أداة مسموحة داخله.
// الصلاحيات تشكّل ما يراه المستخدم داخل «موقف المشروع» وما يمكنه الانتقال إليه بعد ذلك،
// لكنها لا تغيّر معنى الدخول إلى المكان ولا ترمي المستخدم مباشرة في الحضور أو أي وظيفة أخرى.
export function preferredProjectHref(_session, projectId) {
