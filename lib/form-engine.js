...
// دعم parties من المحرك التاريخي محفوظ هنا قبل إزالة النسخة القديمة، حتى لا نفقد أي فائدة.
export function allKeys(layout) {
  ...
  (layout?.sections || []).forEach((s) => {
    ...
    if (s.kind === 'parties' && s.key) keys.push({ key:s.key, label:s.title || s.key, type:'text', where:'بطاقات أطراف' });
  });
  return keys;
}
