...
// v2 intentionally drops stale logical history/origins created before strict portal-section parenting.
const KEY='arkan.navigation.stack.v2';
const BACK_FLAG='arkan.navigation.back-target.v2';
const ORIGIN_KEY='arkan.navigation.origins.v2';
...
function workspaceSectionParent(pathname=''){
  ...
  const match=path.match(/^\/dashboard\/workspace\/(projects|workforce|finance|documents|admin)\/(?:section\/[^/]+|approvals)\/?$/);
...
function forgetNavigationOrigin(currentPath){
...
function commitBackTarget(path,target){
  const storage=safeSession();
  if(!storage)return target;
  const targetPath=pathnameOnly(target);
  const stack=read();
  while(stack.length&&stack.at(-1)===path)stack.pop();
  while(stack.length&&stack.at(-1)!==targetPath&&logicalParentFor(stack.at(-1))===targetPath)stack.pop();
...
// تحفظ الشاشة التي ضغط المستخدم منها على الأداة نفسها. هذه هي أولوية الرجوع للأدوات المرنة فقط.
// أقسام البوابات ومركز الاعتمادات ليست أدوات مرنة: الأب هو البوابة حصريًا.
export function rememberNavigationOrigin(destination,source){
  ...
  if(workspaceSectionParent(destPath)) return;
...
export function logicalBackTarget(currentPath,fallback=WORKSPACE){
  ...
  // قاعدة قطعية: أي مستوى ثابت داخل البوابة يرجع إلى البوابة التي ينتمي إليها دائمًا.
  const portalSectionParent=workspaceSectionParent(path);
  if(portalSectionParent){
    forgetNavigationOrigin(path);
    return commitBackTarget(path,portalSectionParent);
  }

  // للأدوات التي يمكن فتحها من أكثر من موضع فقط: الرجوع إلى شاشة الانطلاق الفعلية بضغطة واحدة.
  ...
  if(directParent){
    return commitBackTarget(path,directParent);
  }
