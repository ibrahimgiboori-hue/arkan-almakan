...
export const ATTEND_CYCLE = Object.freeze([
  SYSTEM.attendance.states.full,
  SYSTEM.attendance.states.half,
  SYSTEM.attendance.states.absent,
]);
...
export const dayName = (d) => {
  if (!d) return '—';
  const value = /^\d{4}-\d{2}-\d{2}$/.test(String(d)) ? `${d}T12:00:00+03:00` : d;
  const x = new Date(value);
  if (Number.isNaN(x.getTime())) return '—';
  return new Intl.DateTimeFormat(SYSTEM.locale, { weekday:'long', timeZone:SYSTEM.timezone }).format(x);
};
