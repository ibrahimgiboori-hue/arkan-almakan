...
export const SYSTEM_VERSION = '2.9.0-constitution';
...
export const CONSTITUTION = Object.freeze({
  ...
  operatingBudget: Object.freeze({
    id: 'company-operating-budget-v2',
...
export function canCancelWorkflow(status) {
  return ![WORKFLOW_STATUS.REJECTED, WORKFLOW_STATUS.CANCELLED, CLAIM_STATUS.PAID].includes(status);
}

export function printMarginStyle(overrides = {}) {
  const m = { ...SYSTEM.print.marginsMm, ...overrides };
  return {
    '--print-margin-top': `${m.top}mm`, '--print-margin-right': `${m.right}mm`,
    '--print-margin-bottom': `${m.bottom}mm`, '--print-margin-left': `${m.left}mm`,
  };
}
