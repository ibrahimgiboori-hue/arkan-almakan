...
export default function ExternalPayrollWorkspace(){
  ...
  async function ensureWorkspace(item,sourceDays){
    ...
    if(!currentBatch){
      ...
      const created=await supabase.from('hr_external_payroll_batches').insert({
        ...
        include_overtime:previous.data?.include_overtime??true,
        include_time_shortage:previous.data?.include_time_shortage??true,
  ...
  function setNationality(profileId,value){
    setProfiles((list)=>list.map((p)=>{
      if(p.id!==profileId)return p;
      if(value==='saudi')return {...p,nationality_category:value,social_insurance_scheme:['saudi_legacy','saudi_new'].includes(p.social_insurance_scheme)?p.social_insurance_scheme:'saudi_legacy'};
      if(value==='gcc')return {...p,nationality_category:value,social_insurance_scheme:'manual'};
      return {...p,nationality_category:value,social_insurance_scheme:null};
    }));
    setDirty(true);
  }
  function setInsuranceActive(profileId,checked){
    setProfiles((list)=>list.map((p)=>{
      if(p.id!==profileId)return p;
      let scheme=p.social_insurance_scheme;
      if(checked&&p.nationality_category==='saudi'&&!['saudi_legacy','saudi_new'].includes(scheme))scheme='saudi_legacy';
      if(checked&&p.nationality_category==='gcc')scheme='manual';
      if(checked&&p.nationality_category==='non_saudi')scheme=null;
      return {...p,social_insurance_active:checked,social_insurance_scheme:scheme};
    }));
    setDirty(true);
  }
  ...
  function openProfile(key){
    ...
    setEditKey(key);setProfileDraft({...profile,show_job_title:!!line.show_job_title,show_identity:!!line.show_identity,manual_additions:line.manual_additions??0,manual_additions_reason:line.manual_additions_reason||'',manual_deductions:line.manual_deductions??0,manual_deductions_reason:line.manual_deductions_reason||'',gosi_employee_rate_override:line.gosi_employee_rate_override??''});
  }
  ...
  async function saveProfile(){
    ...
    try{
      const pq=await supabase.from('hr_client_external_employee_profiles').update({display_employee_no:String(draft.display_employee_no||'').trim()||null,display_name:String(draft.display_name||'').trim(),job_title:String(draft.job_title||'').trim()||null,identity_no:String(draft.identity_no||'').trim()||null,default_payment_method:draft.default_payment_method||null,nationality_category:draft.nationality_category||null,social_insurance_active:!!draft.social_insurance_active,social_insurance_scheme:draft.social_insurance_scheme||null,updated_at:new Date().toISOString()}).eq('id',draft.id).select('*').single();if(pq.error)throw pq.error;
      const lq=await supabase.from('hr_external_payroll_lines').update({show_job_title:!!draft.show_job_title,show_identity:!!draft.show_identity,manual_additions:Number(draft.manual_additions||0),manual_additions_reason:String(draft.manual_additions_reason||'').trim()||null,manual_deductions:Number(draft.manual_deductions||0),manual_deductions_reason:String(draft.manual_deductions_reason||'').trim()||null,gosi_employee_rate_override:numberOrNull(draft.gosi_employee_rate_override),calculated_at:null,calculated_final_net_salary:null,calculation_snapshot:{},updated_at:new Date().toISOString()}).eq('id',line.id).select('*').single();if(lq.error)throw lq.error;
      setProfiles((list)=>list.map((p)=>p.id===draft.id?pq.data:p));setLines((list)=>list.map((item)=>item.id===line.id?lq.data:item));setEditKey('');setProfileDraft(null);setDirty(false);setMsg('تم الحفظ. أعد احتساب الرواتب.');
  ...
  async function exportSalaryTemplate(){
    ...
    try{
      ...
      const info=wb.addWorksheet('تعليمات');info.views=[{rightToLeft:true}];info.getColumn(1).width=115;info.getCell('A1').value='أدخل الجنسية ومكونات الراتب. فعّل «مسجل في التأمينات» فقط للمشترك المسجل. صافي الراتب لا يُدخل يدويًا؛ يحسبه البرنامج من الأساسي والبدلات بعد خصم حصة الموظف عند انطباقها. الأجر الخاضع للاشتراك = الأساسي + بدل السكن.';info.getCell('A1').alignment={wrapText:true,horizontal:'right'};
  ...
  async function importSalaryFile(file){
    ...
    try{
      ...
      const expected=['رقم الموظف','اسم الموظف','الجنسية','مسجل في التأمينات','نظام الاشتراك','نسبة الموظف %','الراتب الأساسي','بدل السكن','بدل النقل','بدلات أخرى','ساعات اليوم','طريقة الدفع','المسمى الوظيفي','الهوية / الإقامة','إضافة','سبب الإضافة','خصم','سبب الخصم','__source_key'];
      ...
      for(let r=2;r<=ws.lastRow.number;r++){
        ...
        if(!nationality)throw new Error(`صف ${r}: حدد الجنسية.`);if(insured===null)throw new Error(`صف ${r}: حدد هل الموظف مسجل في التأمينات.`);
        if(insured&&nationality==='saudi'&&!['saudi_legacy','saudi_new'].includes(scheme))throw new Error(`صف ${r}: اختر نظام اشتراك الموظف السعودي.`);
        if(insured&&nationality==='gcc')scheme='manual';if(nationality==='non_saudi')scheme=null;
        const pq=await supabase.from('hr_client_external_employee_profiles').update({display_employee_no:cellText(row.getCell(1))||profile.display_employee_no||null,display_name:cellText(row.getCell(2))||profile.display_name,job_title:cellText(row.getCell(13))||null,identity_no:cellText(row.getCell(14))||null,default_payment_method:payment||profile.default_payment_method||null,nationality_category:nationality,social_insurance_active:insured,social_insurance_scheme:insured?scheme:null,updated_at:new Date().toISOString()}).eq('id',profile.id);if(pq.error)throw pq.error;
        const draftLine={...line,basic_salary:numberOrNull(cellText(row.getCell(7))),housing_allowance:numberOrNull(cellText(row.getCell(8))),transport_allowance:numberOrNull(cellText(row.getCell(9))),other_allowances:numberOrNull(cellText(row.getCell(10))),gosi_employee_rate_override:numberOrNull(cellText(row.getCell(6)))};
        const breakdown=salaryBreakdown({line:draftLine,profile:{...profile,nationality_category:nationality,social_insurance_active:insured,social_insurance_scheme:insured?scheme:null},periodFrom:activeImport?.period_from});
        const lq=await supabase.from('hr_external_payroll_lines').update({reference_net_salary:breakdown.ready?breakdown.referenceNetSalary:null,basic_salary:draftLine.basic_salary,housing_allowance:draftLine.housing_allowance,transport_allowance:draftLine.transport_allowance,other_allowances:draftLine.other_allowances,gosi_employee_rate_override:draftLine.gosi_employee_rate_override,day_hours_override:numberOrNull(cellText(row.getCell(11))),payment_method:payment||line.payment_method||null,manual_additions:Number(numberOrNull(cellText(row.getCell(15)))||0),manual_additions_reason:cellText(row.getCell(16))||null,manual_deductions:Number(numberOrNull(cellText(row.getCell(17)))||0),manual_deductions_reason:cellText(row.getCell(18))||null,calculated_at:null,calculated_final_net_salary:null,calculation_snapshot:{},updated_at:new Date().toISOString()}).eq('id',line.id);if(lq.error)throw lq.error;changed+=1;
      }
    ...
    {batch&&<>
      <div className="section"><header><h2>بيانات الرواتب</h2><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button className="btn ghost" disabled={busy} onClick={exportSalaryTemplate}>تنزيل نموذج Excel</button><button className="btn ghost" disabled={busy} onClick={()=>salaryFileRef.current?.click()}>استيراد Excel</button><input ref={salaryFileRef} type="file" accept=".xlsx" style={{display:'none'}} onChange={(e)=>importSalaryFile(e.target.files?.[0])}/><button className="btn" disabled={busy||!dirty} onClick={()=>saveInputs(true)}>حفظ</button></div></header><div style={{padding:18,overflowX:'auto'}}><table><thead><tr><th>الرقم</th><th>الموظف</th><th>الجنسية</th><th>التأمينات</th><th>الأساسي</th><th>السكن</th><th>النقل</th><th>بدلات أخرى</th><th>خصم التأمينات</th><th>صافي الراتب</th><th>ساعات اليوم</th><th>طريقة الدفع</th><th>تفاصيل</th></tr></thead><tbody>{people.map((person)=>{
        ...
        return <tr key={person.key}><td><strong>{profile.display_employee_no||person.no||'—'}</strong></td><td><strong>{profile.display_name||person.name}</strong>{profile.display_name!==profile.source_employee_name&&profile.source_employee_name?<div className="hint">{profile.source_employee_name}</div>:null}</td>
          <td><select value={profile.nationality_category||''} onChange={(e)=>setNationality(profile.id,e.target.value||null)} style={{minWidth:120}}><option value="">اختر</option>{NATIONALITY_CATEGORIES.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></td>
          <td style={{minWidth:180}}><label style={{display:'flex',gap:7,alignItems:'center'}}><input type="checkbox" checked={!!profile.social_insurance_active} onChange={(e)=>setInsuranceActive(profile.id,e.target.checked)}/> مسجل</label>{profile.social_insurance_active&&profile.nationality_category==='saudi'&&<select value={profile.social_insurance_scheme||'saudi_legacy'} onChange={(e)=>setProfileField(profile.id,'social_insurance_scheme',e.target.value)} style={{minWidth:150,marginTop:6}}><option value="saudi_legacy">النظام القائم</option><option value="saudi_new">النظام الجديد</option></select>}{profile.social_insurance_active&&profile.nationality_category==='gcc'&&<input type="number" min="0" max="100" step="0.01" placeholder="نسبة الموظف %" value={line.gosi_employee_rate_override??''} onChange={(e)=>setLineField(line.id,'gosi_employee_rate_override',e.target.value)} style={{minWidth:130,marginTop:6}}/>}{profile.social_insurance_active&&profile.nationality_category==='non_saudi'&&<div className="hint" style={{marginTop:5}}>حصة الموظف 0%</div>}</td>
          ...
          <td><strong>{breakdown.ready?`${formatMoney(breakdown.gosiEmployeeDeduction)} ر.س`:'—'}</strong>{breakdown.ready&&<div className="hint">{breakdown.socialInsuranceActive?`${Number(breakdown.gosiEmployeeRate||0).toFixed(2)}%`:'غير مطبق'}</div>}</td>
      ...
      })}</tbody></table><div className="rowsplit" style={{marginTop:12}}><span className="hint">صافي الراتب يُحتسب تلقائيًا. الأجر الخاضع للاشتراك = الأساسي + بدل السكن. لا يخصم من الموظف شيء إذا لم يكن مسجلًا في التأمينات.</span>{salaryMissingCount>0?<strong style={{color:'#8B2E2E'}}>متبقي {salaryMissingCount}</strong>:<strong>مكتمل</strong>}</div></div></div>

      <div className="section"><header><h2>سياسة الاحتساب</h2></header><div style={{padding:18}}><div className="form-grid">
        <div className="field"><label>قيمة اليوم</label><select value={batch.divisor_policy} onChange={(e)=>setBatchField('divisor_policy',e.target.value)}><option value="thirty">الراتب ÷ 30</option><option value="calendar_days">الراتب ÷ أيام الشهر</option></select></div>
        <div className="field"><label>معالجة فرق الساعات</label><div style={{display:'flex',gap:16,alignItems:'center',minHeight:38,flexWrap:'wrap'}}><label style={{display:'flex',gap:7,alignItems:'center'}}><input type="checkbox" checked={batch.include_overtime!==false} onChange={(e)=>setBatchField('include_overtime',e.target.checked)}/> الساعات الإضافية</label><label style={{display:'flex',gap:7,alignItems:'center'}}><input type="checkbox" checked={batch.include_time_shortage!==false} onChange={(e)=>setBatchField('include_time_shortage',e.target.checked)}/> نقص الساعات</label></div></div>
        <div className="field"><label>جزاء البصمة المفقودة</label><div style={{display:'flex',alignItems:'center',gap:8}}><input type="number" min="0" step="0.25" value={batch.missing_punch_deduction_days} onChange={(e)=>setBatchField('missing_punch_deduction_days',e.target.value)}/><span className="hint">يوم / حالة مرفوضة</span></div></div>
        <div className="field"><label>طريقة الدفع الافتراضية</label><select value={batch.default_payment_method||''} onChange={(e)=>setBatchField('default_payment_method',e.target.value||null)}><option value="">لكل موظف</option>{PAYMENT_METHODS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div>
      </div><div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:12}}><button className="btn ghost" disabled={busy||!dirty} onClick={()=>saveInputs(true)}>حفظ</button><button className="btn ghost" disabled={busy} onClick={()=>letterheadRef.current?.click()}>{batch.client_letterhead_path?'استبدال مطبوعات العميل':'رفع مطبوعات العميل'}</button><input ref={letterheadRef} type="file" accept="image/png,image/jpeg,image/webp" style={{display:'none'}} onChange={(e)=>uploadLetterhead(e.target.files?.[0])}/></div></div></div>
