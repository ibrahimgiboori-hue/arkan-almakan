...
export default function ExternalPayrollWorkspace(){
  ...
  async function exportSalaryTemplate(){
    ...
    try{
      ...
      const info=wb.addWorksheet('تعليمات');info.views=[{rightToLeft:true}];info.getColumn(1).width=115;info.getCell('A1').value='أدخل الجنسية ومكونات الراتب. فعّل «مسجل في التأمينات» فقط للمشترك المسجل. صافي الراتب لا يُدخل يدويًا؛ يحسبه البرنامج من الأساسي والبدلات بعد خصم حصة الموظف عند انطباقها. الأجر الخاضع للاشتراك = الأساسي + بدل السكن. لغير السعودي لا توجد حصة موظف في فرع الأخطار المهنية؛ وللسعودي اختر النظام القائم أو الجديد. للخليجي استخدم النسبة المخصصة وفق حالته.';info.getCell('A1').alignment={wrapText:true,horizontal:'right'};
