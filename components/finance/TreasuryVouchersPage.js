...
const EMPTY_VOUCHER={
  voucher_type:'receipt',voucher_date:'',account_id:'',project_id:'',party_name:'',party_id_kind:'',party_id_number:'',party_mobile:'',party_address:'',amount:'',payment_method:'cash',bank_name:'',payment_reference:'',payment_date:'',description:'',supporting_reference:''
};

export default function TreasuryVouchersPage(){
  const me=useDashboardSession();
  const [state,setState]=useState({loading:true,accounts:[],books:[],vouchers:[],projects:[],settings:null,error:''});
  ...
  async function load(){
    ...
    const [accountsQ,booksQ,vouchersQ,projectsQ,settingsQ]=await Promise.all([
      ...
      supabase.from('projects').select('id,project_no,name_ar,status').order('project_no'),
    ...
    setState({loading:false,accounts,books:booksQ.data||[],vouchers:vouchersQ.data||[],projects:projectsQ.error?[]:(projectsQ.data||[]),settings:settingsQ.error?null:(settingsQ.data||null),error:''});
  ...
  const projectMap=useMemo(()=>new Map(state.projects.map((item)=>[item.id,item])),[state.projects]);
  ...
  function nextBook(type){
    ...
    if(!latest)return {book:1,page:1};
  ...
  function printVoucher(voucher){
    const settings=state.settings||{};const account=accountMap.get(voucher.account_id)||{};const project=projectMap.get(voucher.project_id)||{};
    ...
    popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${esc(TYPE_LABEL[voucher.voucher_type])} ${esc(voucher.voucher_no)}</title><style>
      @page{size:A4 portrait;margin:12mm}*{box-sizing:border-box}body{font-family:Tahoma,Arial,sans-serif;color:#2c2c2c;margin:0;background:#fff;font-size:13px}.sheet{border:2px solid #7a1f2b;padding:18px;min-height:260mm;position:relative}.head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #7a1f2b;padding-bottom:12px}.company h1{font-size:20px;color:#7a1f2b;margin:0 0 6px}.company p{margin:2px 0;color:#555}.title{text-align:center;margin:20px 0 14px}.title h2{display:inline-block;margin:0;padding:8px 32px;border:2px solid #7a1f2b;color:#7a1f2b;font-size:24px}.grid{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid #777;margin:12px 0}.cell{padding:8px;border:1px solid #aaa;min-height:52px}.cell b{display:block;color:#7a1f2b;margin-bottom:5px}.wide{grid-column:span 2}.full{grid-column:1/-1}.amount{font-size:20px;font-weight:bold}.legal{font-size:17px;line-height:2;border:1px solid #777;padding:16px;margin:16px 0;background:#faf7f5}.signatures{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:34px}.sign{height:90px;border-top:1px solid #555;text-align:center;padding-top:8px}.foot{position:absolute;bottom:12px;right:18px;left:18px;display:flex;justify-content:space-between;border-top:1px solid #aaa;padding-top:7px;font-size:11px;color:#666}.void{position:absolute;inset:42% 10% auto;transform:rotate(-18deg);font-size:90px;font-weight:bold;color:rgba(180,0,0,.18);text-align:center;z-index:2}@media print{button{display:none}.sheet{border-color:#000}.title h2,.company h1,.cell b{color:#000}}</style></head><body><div class="sheet">${voucher.status==='void'?'<div class="void">ملغى</div>':''}<div class="head"><div class="company"><h1>${esc(company)}</h1><p>${esc(settings.company_name_en||'Arkan Al Makan Contracting Company')}</p><p>س.ت: ${esc(settings.cr_number||'—')} | الرقم الضريبي: ${esc(settings.vat_number||'—')}</p><p>${esc(settings.national_address||settings.city||'الرياض')}</p></div><div><b>رقم الدفتر: ${esc(voucher.book_no)}</b><br><b>رقم السند: ${esc(pageNo(voucher.page_no))}</b><br><b>الرقم المرجعي: ${esc(voucher.voucher_no)}</b></div></div><div class="title"><h2>${esc(TYPE_LABEL[voucher.voucher_type])}</h2></div><div class="grid"><div class="cell"><b>التاريخ</b>${esc(voucher.voucher_date)}</div><div class="cell"><b>رقم الدفتر</b>${esc(voucher.book_no)}</div><div class="cell"><b>رقم السند / الصفحة</b>${esc(pageNo(voucher.page_no))}</div><div class="cell"><b>الحساب</b>${esc(account.name_ar||'—')}</div><div class="cell wide"><b>المبلغ رقمًا</b><span class="amount">${esc(money(voucher.amount))}</span></div><div class="cell wide"><b>المبلغ تفقيطًا</b>${esc(voucher.amount_words)} فقط لا غير</div></div><div class="legal">${esc(voucher.legal_text_snapshot)}</div><div class="grid"><div class="cell wide"><b>${esc(partyTitle)}</b>${esc(voucher.party_name)}</div><div class="cell"><b>نوع الهوية</b>${esc(ID_LABEL[voucher.party_id_kind]||'—')}</div><div class="cell"><b>رقم الهوية / السجل</b>${esc(voucher.party_id_number||'—')}</div><div class="cell"><b>الجوال</b>${esc(voucher.party_mobile||'—')}</div><div class="cell"><b>طريقة الدفع</b>${esc(METHOD_LABEL[voucher.payment_method]||voucher.payment_method)}</div><div class="cell"><b>مرجع الدفع</b>${esc(voucher.payment_reference||'—')}</div><div class="cell"><b>البنك</b>${esc(voucher.bank_name||'—')}</div><div class="cell wide"><b>البيان / السبب</b>${esc(voucher.description)}</div><div class="cell"><b>المرجع المؤيد</b>${esc(voucher.supporting_reference||'—')}</div><div class="cell"><b>المشروع</b>${esc(project.name_ar?`${project.project_no||''} - ${project.name_ar}`:'—')}</div><div class="cell full"><b>العنوان</b>${esc(voucher.party_address||'—')}</div></div><div class="signatures"><div class="sign">${esc(partySignature)}<br><br>الاسم والتوقيع</div><div class="sign">أمين الصندوق<br><br>الاسم والتوقيع</div><div class="sign">المحاسب<br><br>الاسم والتوقيع</div><div class="sign">المعتمد<br><br>الاسم والتوقيع</div></div><div class="foot"><span>الأصل للطرف — نسخة للحسابات — نسخة بالدفتر</span><span>حالة السند: ${voucher.status==='void'?'ملغى':'ساري'}</span></div></div><script>window.onload=()=>{window.print();}</script></body></html>`);
  ...
  return <ConstitutionPage>
    <PageHeader title="الخزينة وسندات القبض والصرف" description="دفاتر مرقمة من 001 إلى 100، مع ترقيم مستقل للقبض والصرف وربط كل سند بحركة الخزينة." actions={<div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{canReceipt?<button className="btn" onClick={()=>openEntry('receipt')}>سند قبض جديد</button>:null}{canPayment?<button className="btn" onClick={()=>openEntry('payment')}>سند صرف جديد</button>:null}{canCreateAccount?<button className="btn ghost" onClick={()=>{setShowEntry(false);setShowAccountEntry(true);setMessage('');}}>إضافة حساب خزينة</button>:null}</div>}/>
