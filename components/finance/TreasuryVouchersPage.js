...
const EMPTY_VOUCHER={
  voucher_type:'receipt',voucher_date:'',account_id:'',page_no:'',party_type:'individual',party_name:'',party_id_kind:'national_id',party_id_number:'',party_nationality:'سعودي',party_title:'',party_representative_name:'',party_representative_title:'',party_mobile:'',party_address:'',amount:'',payment_method:'cash',bank_name:'',payment_reference:'',payment_date:'',description:'',issuer_employee_id:'',approved_by_employee_id:'',accountant_employee_id:''
};
...
export default function TreasuryVouchersPage(){
  ...
  function openEntry(type){
    ...
    setForm({...EMPTY_VOUCHER,voucher_type:type,voucher_date:today(),account_id:state.accounts[0]?.id||'',page_no:String(suggested||1),issuer_employee_id:'',approved_by_employee_id:defaultManager?.id||'',accountant_employee_id:defaultAccountant?.id||''});
  ...
  async function issueVoucher(event){
    ...
    if(!form.account_id||!form.party_name.trim()||!form.description.trim()||!amountWords||!form.approved_by_employee_id||!form.accountant_employee_id){setMessage('أكمل الحساب والطرف والمبلغ وسبب السند والمحاسب والمعتمد المطلوب.');return;}
    setBusy(true);
    const params={
