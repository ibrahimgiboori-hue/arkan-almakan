...
const LEGAL_TEXT_DEFAULT={
  receipt:'وأقر أنا المحصل منه المذكور أعلاه بأنني سلمت المبلغ المبين في هذا السند رقمًا وكتابةً وفق البيانات الموضحة أعلاه، وأن توقيعي يعد إقرارًا بصحة عملية القبض في حدود هذا السند.',
  payment:'وأقر أنا المستفيد المذكور أعلاه باستلام كامل المبلغ المبين في هذا السند رقمًا وكتابةً عن الاستحقاق الموضح أعلاه، بعد الاطلاع على بياناته والعلم بسبب الصرف وطريقة الوفاء، ويعد توقيعي إقرارًا بصحة الاستلام في حدود هذا السند، دون أن يعد إبراءً عامًا عن أي حقوق أو التزامات أخرى.'
};
...
const EMPTY_VOUCHER={
  voucher_type:'receipt',voucher_date:'',account_id:'',page_no:'',party_type:'individual',party_name:'',party_id_kind:'national_id',party_id_number:'',party_nationality:'سعودي',party_title:'',party_representative_name:'',party_representative_title:'',party_mobile:'',party_address:'',amount:'',payment_method:'cash',bank_name:'',payment_reference:'',payment_date:'',description:'',legal_text:'',issuer_employee_id:'',approved_by_employee_id:'',accountant_employee_id:''
};
...
export default function TreasuryVouchersPage(){
  ...
  async function load(){
    ...
    const defaultAccountant=actors.find((item)=>item.full_name_ar==='اسلام ابراهيم')||actors.find((item)=>/مراقب مالي|محاسب/.test(item.job_title||''))||null;
    ...
    setForm((current)=>({...current,account_id:current.account_id||accounts[0]?.id||'',approved_by_employee_id:current.approved_by_employee_id||defaultManager?.id||'',accountant_employee_id:current.accountant_employee_id||defaultAccountant?.id||''}));
  }
  ...
  function openEntry(type){
    ...
    const defaultAccountant=state.actors.find((item)=>item.full_name_ar==='اسلام ابراهيم')||state.actors.find((item)=>/مراقب مالي|محاسب/.test(item.job_title||''))||null;
    const suggested=type==='receipt'?nextReceipt.page:nextPayment.page;
    setEditingVoucherId(null);
    setForm({...EMPTY_VOUCHER,voucher_type:type,voucher_date:today(),account_id:state.accounts[0]?.id||'',page_no:String(suggested||1),legal_text:LEGAL_TEXT_DEFAULT[type]||'',issuer_employee_id:'',approved_by_employee_id:defaultManager?.id||'',accountant_employee_id:defaultAccountant?.id||''});
  ...
  function openEdit(voucher){
    ...
    setForm({
      ...
      party_type:voucher.party_type||(voucher.party_id_kind==='cr'?'establishment':'individual'),
      party_name:voucher.party_name||'',
      party_id_kind:voucher.party_id_kind||'national_id',
      party_id_number:voucher.party_id_number||'',
      party_nationality:voucher.party_nationality||'',
      party_title:voucher.party_title||'',
      party_representative_name:voucher.party_representative_name||'',
      party_representative_title:voucher.party_representative_title||'',
      ...
      legal_text:voucher.legal_text_snapshot||LEGAL_TEXT_DEFAULT[voucher.voucher_type]||'',
      issuer_employee_id:voucher.issuer_employee_id||'',
      approved_by_employee_id:voucher.approved_by_employee_id||'',
      accountant_employee_id:voucher.accountant_employee_id||'',
    });
    setMessage('');setShowAccountEntry(false);setShowEntry(true);
  }
  ...
  async function issueVoucher(event){
    ...
    if(error){setBusy(false);setMessage(errorMessage(error));return;}
    const voucherId=editingVoucherId||data?.id;
    if(voucherId){
      const profile=await supabase.rpc('fn_cash_voucher_set_party_profile',{
        p_voucher_id:voucherId,
        p_party_type:form.party_type||'individual',
        p_party_nationality:form.party_type==='individual'?(form.party_nationality||null):null,
        p_party_title:form.party_type==='individual'?(form.party_title||null):null,
        p_party_representative_name:form.party_type==='establishment'?(form.party_representative_name||null):null,
        p_party_representative_title:form.party_type==='establishment'?(form.party_representative_title||null):null
      });
      if(profile.error){setBusy(false);setMessage(errorMessage(profile.error));return;}
      const legal=await supabase.rpc('fn_cash_voucher_set_legal_text',{
        p_voucher_id:voucherId,
        p_legal_text:form.legal_text.trim()
      });
      if(legal.error){setBusy(false);setMessage(errorMessage(legal.error));return;}
    }
    setBusy(false);
  ...
  return <ConstitutionPage>
    ...
    {showEntry?<EntrySurface title={editingVoucherId?`تعديل مسودة ${TYPE_LABEL[form.voucher_type]}`:(form.voucher_type==='receipt'?'مسودة سند قبض جديد':'مسودة سند صرف جديد')} description={editingVoucherId?'يمكن تعديل جميع بيانات المسودة قبل إرسالها للتعميد.':`الدفتر ${form.voucher_type==='receipt'?nextReceipt.book:nextPayment.book} — اختيار رقم السند يحجزه للمسودة، ولا ينشأ أثر مالي حتى التعميد النهائي.`} actions={<button className="btn ghost" type="button" onClick={()=>{setEditingVoucherId(null);setShowEntry(false);}}>إغلاق</button>}><form onSubmit={issueVoucher}><div className="form-grid"><div className="field"><label>التاريخ *</label><input type="date" value={form.voucher_date} onChange={(e)=>field('voucher_date',e.target.value)} required/></div><div className="field"><label>رقم السند الورقي *</label><select value={form.page_no} onChange={(e)=>field('page_no',e.target.value)} required><option value="">اختر</option>{Array.from({length:100},(_,i)=>i+1).map((n)=>{const used=usedPagesFor(form.voucher_type,editingVoucherId).has(n);return <option key={n} value={n} disabled={used}>{pageNo(n)}{used?' — مستخدم':''}</option>;})}</select></div><div className="field"><label>حساب الصندوق / البنك *</label><select value={form.account_id} onChange={(e)=>field('account_id',e.target.value)} required><option value="">اختر</option>{state.accounts.map((item)=><option key={item.id} value={item.id}>{item.name_ar} — {money(item.current_balance)}</option>)}</select></div><div className="field"><label>نوع الطرف *</label><select value={form.party_type} onChange={(e)=>{const type=e.target.value;setForm((v)=>({...v,party_type:type,party_id_kind:type==='establishment'?'cr':(v.party_id_kind==='cr'?'national_id':v.party_id_kind),party_nationality:type==='establishment'?'':(v.party_nationality||'سعودي')}));}}><option value="individual">فرد</option><option value="establishment">منشأة</option></select></div><div className="field"><label>{form.party_type==='establishment'?'اسم المنشأة *':(form.voucher_type==='receipt'?'اسم الطرف *':'اسم المستفيد *')}</label><input value={form.party_name} onChange={(e)=>field('party_name',e.target.value)} required/></div>{form.party_type==='individual'?<div className="field"><label>الجنسية</label><select value={form.party_nationality} onChange={(e)=>{const nationality=e.target.value;setForm((v)=>({...v,party_nationality:nationality,party_id_kind:nationality==='سعودي'?'national_id':'iqama'}));}}><option value="سعودي">سعودي</option><option value="غير سعودي">غير سعودي</option></select></div>:null}<div className="field"><label>المبلغ رقمًا *</label><input type="number" min="0.01" step="0.01" value={form.amount} onChange={(e)=>field('amount',e.target.value)} required/></div><div className="field" style={{gridColumn:'1/-1'}}><label>المبلغ تفقيطًا</label><textarea rows={2} value={amountWords?`${amountWords} فقط لا غير`:''} readOnly/></div><div className="field"><label>{form.party_type==='establishment'?'السجل التجاري':form.party_nationality==='سعودي'?'رقم الهوية':'رقم الإقامة'}</label><input value={form.party_id_number} onChange={(e)=>field('party_id_number',e.target.value)}/></div>{form.party_type==='individual'?<div className="field"><label>الصفة</label><input value={form.party_title} onChange={(e)=>field('party_title',e.target.value)} placeholder="مثال: موظف"/></div>:<><div className="field"><label>يمثلها</label><input value={form.party_representative_name} onChange={(e)=>field('party_representative_name',e.target.value)} placeholder="اسم ممثل المنشأة"/></div><div className="field"><label>صفة الممثل</label><input value={form.party_representative_title} onChange={(e)=>field('party_representative_title',e.target.value)} placeholder="مثال: المدير العام"/></div></>}<div className="field"><label>رقم الجوال</label><input value={form.party_mobile} onChange={(e)=>field('party_mobile',e.target.value)}/></div><div className="field"><label>مكان الوفاء</label><input value={form.party_address} onChange={(e)=>field('party_address',e.target.value)} placeholder="مثال: الرياض"/></div><div className="field"><label>طريقة الدفع *</label><select value={form.payment_method} onChange={(e)=>field('payment_method',e.target.value)}>{Object.entries(METHOD_LABEL).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div><div className="field"><label>البنك</label><input value={form.bank_name} onChange={(e)=>field('bank_name',e.target.value)}/></div><div className="field"><label>مرجع الدفع</label><input value={form.payment_reference} onChange={(e)=>field('payment_reference',e.target.value)} placeholder="مثال للرواتب: مسير رواتب شهر أغسطس 2026"/></div><div className="field"><label>تاريخ الدفع / الشيك</label><input type="date" value={form.payment_date} onChange={(e)=>field('payment_date',e.target.value)}/></div><div className="field"><label>مصدر السند</label><select value={form.issuer_employee_id} onChange={(e)=>field('issuer_employee_id',e.target.value)}><option value="">مؤسسة اركان المكان للمقاولات</option>{state.actors.map((item)=><option key={item.id} value={item.id}>{item.full_name_ar}{item.job_title?` — ${item.job_title}`:''}</option>)}</select><small>إذا لم يتم اختيار موظف، يكون المصدر مؤسسة اركان المكان للمقاولات.</small></div><div className="field"><label>المحاسب *</label><select value={form.accountant_employee_id} onChange={(e)=>field('accountant_employee_id',e.target.value)} required><option value="">اختر المحاسب</option>{state.actors.map((item)=><option key={item.id} value={item.id}>{item.full_name_ar}{item.job_title?` — ${item.job_title}`:''}</option>)}</select></div><div className="field"><label>المعتمد المطلوب *</label><select value={form.approved_by_employee_id} onChange={(e)=>field('approved_by_employee_id',e.target.value)} required disabled><option value="">اختر</option>{state.actors.map((item)=><option key={item.id} value={item.id}>{item.full_name_ar}{item.job_title?` — ${item.job_title}`:''}</option>)}</select><small>لا يظهر اسمه أو الختم في الطباعة إلا بعد التعميد الفعلي.</small></div><div className="field" style={{gridColumn:'1/-1'}}><label>{form.voucher_type==='receipt'?'سبب القبض *':'سبب الصرف *'}</label><textarea rows={3} value={form.description} onChange={(e)=>field('description',e.target.value)} required/></div><div className="field" style={{gridColumn:'1/-1'}}><label>النص القانوني *</label><textarea rows={4} value={form.legal_text} onChange={(e)=>field('legal_text',e.target.value)} required/><small>هذا النص يطبع داخل السند ويمكن تعديله لكل مسودة قبل التعميد.</small><div style={{marginTop:6}}><button type="button" className="btn ghost" onClick={()=>field('legal_text',LEGAL_TEXT_DEFAULT[form.voucher_type]||'')}>استعادة النص الافتراضي</button></div></div></div><Notice tone="neutral"><strong>معاينة النص:</strong> {form.voucher_type==='receipt'?`استلمنا نحن / ${state.settings?.company_name_ar||'أركان المكان للمقاولات'} سجل تجاري رقم / ${state.settings?.cr_number||'…'} من المكرم / ${form.party_name||'…'} مبلغًا وقدره / ${amountWords||'…'}، وذلك مقابل / ${form.description||'…'}.`:`${form.party_id_kind==='cr'?'استلمنا نحن':'استلمت أنا'} / ${form.party_name||'…'} ${ID_NUMBER_LABEL[form.party_id_kind]||'رقم إثبات'} / ${form.party_id_number||'…'} مكان الوفاء / ${form.party_address||'…'} مبلغًا وقدره / ${amountWords||'…'}، وذلك مقابل / ${form.description||'…'}، وأقر باستلام المبلغ كاملًا.`}</Notice><div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:14}}><button type="button" className="btn ghost" onClick={()=>{setEditingVoucherId(null);setShowEntry(false);}}>إلغاء</button><button className="btn" disabled={busy||!state.accounts.length}>{busy?'جارٍ الحفظ…':editingVoucherId?'حفظ التعديلات':'حفظ المسودة وتثبيت رقمها'}</button></div></form></EntrySurface>:null}
