...
export default function ResizableLayoutGrid({ items = [], onResize, isTable = false, gridColumns = 48 }) {
  ...
  const spans = useMemo(
    () => items.map((item, index) => clamp(Number(draft[index] ?? item.span ?? (isTable ? gridColumns / 3 : gridColumns / 2)), 1, gridColumns)),
    [items, draft, isTable, gridColumns]
  );
  ...
  function startResize(event, index) {
    ...
    const step = rect.width / gridColumns;
    dragRef.current = {
      index,
  ...
  function moveResize(event) {
    ...
    const next = clamp(d.startSpan + Math.round(delta / d.step), 1, gridColumns);
  ...
  function endResize(event) {
    const d = dragRef.current;
    if (!d) return;
    const next = clamp(Number(draft[d.index] ?? spans[d.index]), 1, gridColumns);
    ...
    if (next !== Number(items[d.index]?.span || (isTable ? gridColumns / 3 : gridColumns / 2))) {
  ...
  return (
    <div className="layout-resize-wrap">
      <div className="layout-resize-note">
        اسحب الحافة اليسرى لأي خلية لتغيير عرضها. كل خطوة تمثل وحدة من شبكة A4 المكوّنة من {gridColumns} وحدة،
          ...
          return (
            ...
            >
              ...
              <div className="layout-resize-cell-meta">
                ...
                <strong>{span}/{gridColumns}</strong>
              </div>
              <div
      ...
      <style jsx>{`
        ...
        .layout-resize-grid{position:relative;display:grid;grid-template-columns:repeat(${gridColumns},minmax(0,1fr));grid-auto-flow:row;gap:1px;padding:7px;background-image:linear-gradient(to left,rgba(139,51,50,.09) 1px,transparent 1px);background-size:calc(100% / ${gridColumns}) 100%;border:1px solid #d8caca;min-height:68px;overflow:hidden}
