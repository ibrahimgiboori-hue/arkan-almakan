...
// تطبيع : يضمن بنية سليمة مهما كانت البيانات القادمة ناقصة
const normRow = (r) => ({ label: r?.label ?? '', value: r?.value ?? '' });
const normCard = (c) => ({
  title: c?.title ?? '',
  rows: Array.isArray(c?.rows) && c.rows.length ? c.rows.map(normRow) : [normRow()],
});
const normGroup = (g) => ({
  split: Number(g?.split) || 40,
  midText: g?.midText ?? '',
  cards: Array.isArray(g?.cards) && g.cards.length
    ? g.cards.map(normCard)
    : [normCard({ title: 'الطرف الأول' }), normCard({ title: 'الطرف الثاني' })],
});

export default function PartyCards({ value, onChange, editable = true }) {
  const groups = (Array.isArray(value) && value.length ? value : [partyGroup(2)]).map(normGroup);
