...
-- approval_workflows هو صندوق القرار الرسمي الوحيد، بينما تبقى جداول المصدر
...
create table if not exists public.approval_workflow_stage_policies (
  ...
  target_portal_key text check (
    target_portal_key is null
    or target_portal_key in ('projects','workforce','finance','documents','admin')
  ),
  source_action text,
...
comment on table public.approval_workflow_stage_policies is
  'المراحل الإلزامية داخل محرك approval_workflows؛ وصف لمسار المحرك المرجعي وليس محركًا موازيًا.';
...
(
  ...
  'module:workforce','الموارد البشرية',false,'none',false,true,
  'hr.leaves.create','leave_requests',array['draft','submitted']::text[],
  'submitted','draft','rejected','ceo_approved'
),
(
  'financial_case','اعتماد معاملة مالية','finance','finance.cases.create','finance.cases.approve',
  'module:finance','المالية',false,'mandatory',false,true,
  null,'financial_cases',array['submitted','in_review','returned_to_source']::text[],
  null,null,null,null
)
...
values
  ('leave_request',1,'hr_review','مراجعة الموارد البشرية','capability','hr.leaves.review',
   'module:workforce','الموارد البشرية','workforce','leave_hr_review',false,true),
  ('leave_request',2,'final_approval','الاعتماد النهائي','primary_user',null,
   'primary:system','الاعتماد النهائي','admin','leave_final_approval',true,true),
  ('financial_case',1,'finance_review','المراجعة والاعتماد المالي','capability','finance.cases.approve',
   'module:finance','المالية','finance','financial_review',false,true),
  ('financial_case',2,'final_approval','الاعتماد النهائي','primary_user',null,
   'primary:system','الاعتماد النهائي','admin','financial_final_approval',true,true)
...
-- financial_cases لم يكن مسجلاً كمصدر صريح في سجل مصادر الإجراءات.
-- لا يوجد amount_column في financial_cases؛ مبلغ الاعتماد يؤخذ من financial_case_versions
-- بواسطة المشغل الداخلي أدناه، لذلك لا نعرض financial_case لمسار universal submit العام.
...
values(
  ...
  'معاملة مالية أصلية ذات دورة مراجعة واعتماد وصرف؛ مبلغها الموثوق في financial_case_versions.',
...
begin
  ...
  if v_stage.target_mode='primary_user' then
    ...
    if v_target_user is null
       or not exists(select 1 from public.app_users where id=v_target_user and is_active) then
    ...
    insert into public.approval_workflow_steps(
      ...
      v_stage.target_group_key,v_stage.target_group_label,p_requested_by,
      nullif(trim(coalesce(p_reason,'')),''),false,v_stage.target_portal_key
  ...
  insert into public.approval_workflow_events(
    ...
    jsonb_build_object(
      ...
      'stage_label',v_stage.stage_label_ar
    )
  );
...
revoke all on function private.fn_enqueue_required_approval_stage(uuid,integer,integer,uuid,text)
...
begin
  ...
  case v_stage.source_action
    when 'leave_hr_review' then
      ...
      where id=p_workflow.source_id and status='submitted';
      if not found then raise exception 'حالة طلب الإجازة لا تسمح بمراجعة الموارد البشرية'; end if;

    when 'leave_final_approval' then
      update public.leave_requests
      set status='ceo_approved'
      where id=p_workflow.source_id and status='hr_reviewed';
      if not found then raise exception 'طلب الإجازة لم يجتز مراجعة الموارد البشرية'; end if;

    when 'financial_review' then
      perform public.fn_financial_case_action(
        p_workflow.source_id,'finance_approve',p_comment,null,0,0
      );

    when 'financial_final_approval' then
      perform public.fn_financial_case_action(
        p_workflow.source_id,'final_approve',p_comment,null,0,0
      );
...
revoke all on function private.fn_apply_required_approval_stage_effect(
  public.approval_workflows,public.approval_workflow_steps,text
) from public, anon, authenticated;

-- مسار داخلي للأحداث التي ينشئها المصدر بنفسه (مثل إنشاء/إعادة احتساب نسخة مالية).
-- لا يوجد grant للواجهة، فلا يمكن استخدامه لتجاوز بوابة التسجيل العامة.
...
begin
  ...
  if v_origin_user is null
     or not exists(select 1 from public.app_users where id=v_origin_user and is_active) then
    select primary_user_id into v_origin_user
    from public.system_access_settings
    where singleton=true;
  end if;
  ...
  if v_workflow.id is null then
    ...
    insert into public.approval_workflows(
      workflow_no,transaction_type,source_table,source_id,source_module,source_label,
      project_id,amount,status,version_no,origin_user_id,origin_group_key,origin_group_label,metadata
    ) values(
      v_no,p_transaction_type,p_source_table,p_source_id,v_policy.source_module,p_source_label,
      p_project_id,p_amount,'pending',1,v_origin_user,v_origin.group_key,v_origin.group_label,
      jsonb_build_object('financial_mode',v_policy.financial_mode,'source_event',true)
    ) returning * into v_workflow;
    v_version:=1;
  else
    v_old_version:=v_workflow.version_no;

    if v_workflow.status='pending' then
      update public.approval_workflow_steps
      set status='cancelled',acted_by_user_id=v_origin_user,
          decision_comment='استبدلت بنسخة أحدث من المصدر',acted_at=now()
      ...
      insert into public.approval_workflow_events(
        ...
        v_workflow.id,v_old_version,null,'source_version_superseded',v_origin_user,
        'استبدلت النسخة المعلقة بنسخة أحدث من المصدر',
        jsonb_build_object('old_version',v_old_version,'new_version',v_old_version+1)
      );
    end if;

    v_version:=v_old_version+1;
    update public.approval_workflows
    set status='pending',version_no=v_version,source_label=p_source_label,
        project_id=p_project_id,amount=p_amount,origin_user_id=v_origin_user,
        origin_group_key=v_origin.group_key,origin_group_label=v_origin.group_label,
  ...
  insert into public.approval_workflow_versions(
    workflow_id,version_no,snapshot,amount,submitted_by,note
  ) values(
    v_workflow.id,v_version,coalesce(p_snapshot,'{}'::jsonb),p_amount,v_origin_user,p_note
  );
  ...
  if v_stage.transaction_type is null then
    insert into public.approval_workflow_steps(
      workflow_id,version_no,step_order,target_type,target_capability,target_group_key,
      target_group_label,requested_by_user_id,request_reason,is_additional
    ) values(
      v_workflow.id,v_version,1,'capability',v_policy.initial_target_capability,
      v_policy.initial_target_group_key,v_policy.initial_target_group_label,
      v_origin_user,p_note,false
    ) returning id into v_step;
  else
    perform private.fn_enqueue_required_approval_stage(
      v_workflow.id,v_version,1,v_origin_user,p_note
    );
  ...
  insert into public.approval_workflow_events(
    ...
    v_workflow.id,v_version,v_step,'submitted_from_source',v_origin_user,p_note,
    jsonb_build_object('amount',p_amount,'source_event',true)
  );
...
revoke all on function private.fn_approval_start_from_source_event(
  text,text,uuid,text,uuid,numeric,jsonb,text,uuid
) from public, anon, authenticated;
...
begin
  ...
  v_label:=concat_ws(
    ' — ',nullif(v_case.case_no,''),
    coalesce(nullif(v_case.counterparty_name,''),nullif(v_case.source_label,''),'معاملة مالية')
  );
  ...
  perform private.fn_approval_start_from_source_event(
    ...
    jsonb_build_object('case',to_jsonb(v_case),'financial_version',to_jsonb(new)),
    new.source_note,coalesce(new.submitted_by,v_case.created_by)
  );
...
revoke all on function private.trg_financial_case_version_to_approval()
  from public, anon, authenticated;
...
-- القرار المركزي: الأنواع ذات المراحل الإلزامية تنتقل تلقائيًا للمرحلة التالية.
-- الأنواع القديمة تحتفظ بسلوكها السابق، بما في ذلك الإحالة اليدوية إن سمحت سياستها بذلك.
...
declare
  ...
  v_current_stage public.approval_workflow_stage_policies;
...
begin
  ...
  from public.approval_workflow_stage_policies
  where transaction_type=w.transaction_type
    and stage_order=s.step_order
    and is_active
  limit 1;

  if w.transaction_type='leave_request' then
    v_scenario:=public.leave_request_scenario(w.source_id);
  end if;

  if p_decision in ('return','reject')
  ...
  if p_decision='return' then
    ...
    if w.transaction_type='financial_case' then
      perform public.fn_financial_case_action(
        w.source_id,'return_to_source',p_comment,null,0,0
      );
    else
    ...
    if w.transaction_type='financial_case' then
      perform public.fn_financial_case_action(
        w.source_id,'cancel',p_comment,null,0,0
      );
    else
    ...
    if v_required_next.transaction_type is not null
       and (p_next_user_id is not null or p_next_capability is not null) then
    ...
    if v_required_next.transaction_type is not null then
      perform private.fn_enqueue_required_approval_stage(
        w.id,w.version_no,s.step_order+1,v_uid,p_next_reason
      );
      ...
      if p_next_user_id is not null and (
        exists(
          select 1 from public.approval_workflow_steps x
          where x.workflow_id=w.id and x.version_no=w.version_no
            and x.acted_by_user_id=p_next_user_id
        )
  ...
  insert into public.approvals(
    ...
    approval_method,decision_date,recorded_by_user_id,recorded_at,source,
    stage_code,stage_label_snapshot,action_code,action_label_snapshot,
    ...
    coalesce(v_current_stage.stage_key,'dynamic_approval'),
    coalesce(v_current_stage.stage_label_ar,s.target_group_label),
    p_decision,
    case p_decision when 'approve' then 'اعتماد' when 'reject' then 'رفض' else 'إرجاع للتعديل' end,
    v_final_action,v_scenario,w.id,w.version_no,s.id
  );
  ...
  insert into public.approval_workflow_events(
    ...
    jsonb_build_object(
      ...
      'required_next_stage',case
        when v_required_next.transaction_type is null then null
        else v_required_next.stage_key end
    )
  );
...
revoke all on function public.fn_approval_decide(uuid,text,text,uuid,text,text)
  from public, anon;
grant execute on function public.fn_approval_decide(uuid,text,text,uuid,text,text)
  to authenticated;

-- تفاصيل الصندوق تعرف إن كانت المرحلة الحالية وسيطة أم نهائية.
...
begin
  if auth.uid() is null
     or not private.fn_can_read_approval_workflow(p_workflow_id,auth.uid()) then
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',x.id,'step_order',x.step_order,'target_type',x.target_type,
    'target_user_id',x.target_user_id,'target_capability',x.target_capability,
    'target_group_key',x.target_group_key,'target_group_label',x.target_group_label,
    'request_reason',x.request_reason,'status',x.status,
    'acted_by_user_id',x.acted_by_user_id,'decision_comment',x.decision_comment,
    'acted_at',x.acted_at,'is_additional',x.is_additional
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'event_type',e.event_type,'actor_user_id',e.actor_user_id,'note',e.note,
...
-- إلغاء الإجازة يلغي دورة الاعتماد المفتوحة أيضًا حتى لا يبقى عنصر ميت في الصندوق.
...
begin
  ...
  where transaction_type='leave_request'
    and source_table='leave_requests'
    and source_id=p_id
    and status in ('pending','returned')
  order by created_at desc
  limit 1
  ...
  if v_workflow.id is not null then
    ...
    update public.approval_workflow_steps
    set status='cancelled',acted_by_user_id=auth.uid(),
        decision_comment=nullif(trim(p_reason),''),acted_at=now()
    where workflow_id=v_workflow.id
      and version_no=v_workflow.version_no
      and status='pending';

    update public.approval_workflows
    set status='cancelled',finalized_at=now(),updated_at=now(),
        return_note=nullif(trim(p_reason),'')
    where id=v_workflow.id;

    insert into public.approval_workflow_events(
      workflow_id,version_no,step_id,event_type,actor_user_id,note,payload
    ) values(
      v_workflow.id,v_workflow.version_no,v_step.id,'cancelled',auth.uid(),
  ...
  insert into public.approvals(
    entity_table,entity_id,step_order,step_role,decision,decided_by,decided_at,comment,
    source,stage_code,stage_label_snapshot,action_code,action_label_snapshot,
    is_final_action,scenario_snapshot,workflow_id,workflow_version,workflow_step_id
  ) values(
    'leave_requests',p_id,
    coalesce((select max(step_order)+1 from public.approvals
              where entity_table='leave_requests' and entity_id=p_id),1),
    v_actor_role,'cancelled',auth.uid(),now(),nullif(trim(p_reason),''),
    'live','cancel','إلغاء','cancel','إلغاء الطلب',true,'normal',
...
-- ترحيل الحالات المفتوحة الموجودة قبل التوحيد مرة واحدة فقط.
-- الحالات المغلقة/التاريخية لا يعاد تفسيرها ولا يعاد كتابة تاريخها.
do $$
declare
  r record;
  v_primary uuid;
begin
  select primary_user_id into v_primary
  from public.system_access_settings
  where singleton=true;
  ...
  for r in
    ...
    where lr.record_source<>'historical_paper'
      ...
      and not exists(
        ...
        where w.source_table='leave_requests'
          and w.source_id=lr.id
          and w.transaction_type='leave_request'
      )
  loop
    perform private.fn_approval_start_from_source_event(
      'leave_request','leave_requests',r.id,
      concat_ws(' — ',nullif(r.request_no,''),coalesce(nullif(r.full_name_ar,''),'طلب إجازة')),
      null,null,to_jsonb(r),r.reason,
      coalesce(r.created_by,r.recorded_by_user_id,v_primary)
    );
  end loop;
  ...
  for r in
    ...
    join public.financial_case_versions fv
      on fv.case_id=fc.id and fv.version_no=fc.current_version_no
    where fc.status in ('submitted','in_review')
      and not exists(
        select 1 from public.approval_workflows w
        where w.source_table='financial_cases'
          and w.source_id=fc.id
          and w.transaction_type='financial_case'
      )
  loop
    perform private.fn_approval_start_from_source_event(
      ...
      concat_ws(
        ' — ',nullif(r.case_no,''),
        coalesce(nullif(r.counterparty_name,''),nullif(r.source_label,''),'معاملة مالية')
      ),
      r.project_id,r.requested_amount,
      jsonb_build_object(
        'case',to_jsonb(r)-'source_snapshot'-'requested_amount'-'source_note'-'submitted_by',
        'financial_version_snapshot',r.source_snapshot
      ),
