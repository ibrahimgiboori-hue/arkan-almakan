-- ربط المصروفات التشغيلية اليومية بالملخص المالي المركزي للمشروع.
-- يمنع الازدواج مع المصروفات المنشأة من العهدة والمربوطة بحركة عهدة فعلية.
...
select p.id as project_id,
       ...
       (case when p.supply_scope='labor_only' then 0 else coalesce(m.material_cost,0) end)+coalesce(cu.spent_arkan,0)+coalesce(dx.expense_cost_arkan,0) as direct_cost_known,
       coalesce(e.earned_value,0)-(case when p.supply_scope='labor_only' then 0 else coalesce(m.material_cost,0) end)-coalesce(cu.spent_arkan,0)-coalesce(dx.expense_cost_arkan,0) as current_profit,
