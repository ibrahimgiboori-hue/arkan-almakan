...
begin
  ...
  loop
    if exists (
      select 1
      from public.cash_voucher_books
      where voucher_type = v_book.voucher_type
        and book_no = 2
        and id <> v_book.id
    ) then
      raise exception 'تعذر نقل الدفتر الإلكتروني 1 إلى 2 لوجود دفتر 2 مسبقًا لنوع السند %', v_book.voucher_type;
    end if;
    ...
    set book_no = 2,
        voucher_no = (case when voucher_type = 'receipt' then 'RCV' else 'PYV' end)
          || '-002-' || lpad(page_no::text, 3, '0')
    where book_id = v_book.id;

    update public.treasury_movements tm
    set source_ref = cv.voucher_no
    from public.cash_vouchers cv
    where cv.book_id = v_book.id
      and tm.id = cv.treasury_movement_id;
  end loop;
end;
$migration$;

alter table public.cash_voucher_books
  drop constraint if exists cash_voucher_books_electronic_book_no_check;
alter table public.cash_voucher_books
  add constraint cash_voucher_books_electronic_book_no_check
  check (book_no >= 2);

alter table public.cash_vouchers
  drop constraint if exists cash_vouchers_electronic_book_no_check;
alter table public.cash_vouchers
  add constraint cash_vouchers_electronic_book_no_check
  check (book_no >= 2);
...
begin
  ...
  if v_book.id is null or v_book.next_page > 100 then
    ...
    select greatest(coalesce(max(book_no),1)+1,2) into v_book_no
    from public.cash_voucher_books
