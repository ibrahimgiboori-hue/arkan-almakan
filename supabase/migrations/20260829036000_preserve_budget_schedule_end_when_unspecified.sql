-- تعديل الاسم أو الملاحظات لا يجوز أن يمحو نهاية عقد/التزام محدد المدة.
-- إذا لم يرسل العميل valid_to صراحةً نحافظ على نهاية الجدولة الحالية كما هي.
...
begin
  ...
  if p_schedule is not null then
    ...
    if p_schedule ? 'valid_to' then
      v_sched_valid_to := nullif(p_schedule->>'valid_to','')::date;
    else
      v_sched_valid_to := v_schedule.valid_to;
    end if;
