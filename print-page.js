      ...
      <div className="sheet-wrap">
        <div className="sheet" style={sheetStyle}>
          ...
          {custom && tpl.layout.sections.map((s) => {
            ...
            if (s.kind === 'parties') {
              const groups = Array.isArray(p[s.key]) && p[s.key].length
                ? p[s.key] : (s.groups || []);
              if (!groups.length) return null;
              return (
                <div key={s.id} style={{marginBottom:'6mm'}}>
                  {groups.map((g, gi) => {
                    const per = (g.cards || []).length === 1 ? 1 : 2;
                    const pairs = [];
                    for (let i = 0; i < (g.cards || []).length; i += per) {
                      pairs.push(g.cards.slice(i, i + per));
                    }
                    return (
                      <div key={gi}>
                        {pairs.map((pc, pi) => (
                          <div key={pi}>
                            {pi === 1 && g.midText ? (
                              <div className="dc-body"
                                   style={{textAlign:'center', margin:'3mm 0'}}>
                                {g.midText}
                              </div>
                            ) : null}
                            <div style={{
                              display:'grid',
                              gridTemplateColumns: per === 1 ? '1fr' : '1fr 1fr',
                              gap:'4mm', marginBottom:'3mm',
                            }}>
                              {pc.map((c, ci) => (
                                <section className="card-doc" key={ci}>
                                  <div className="card-head">{c.title}</div>
                                  <table><tbody>
                                    {(c.rows || []).map((r, ri) => (
                                      <tr key={ri}>
                                        <td className="k" style={{width:`${g.split || 40}%`}}>{r.label}</td>
