    ...
    if (t?.is_custom && t?.layout?.sections?.length) {
      ...
      if (!docId) {
        ...
        (t.layout.sections || []).forEach((s) => {
          ...
          if (s.kind === 'parties' && s.key)
            init[s.key] = s.groups?.length ? structuredClone(s.groups) : [partyGroup(2)];
        });
        setV(init);
        ...
        <div className="section" key={s.id}>
          ...
          {s.kind === 'parties' && (
            <div style={{padding:18}}>
              <PartyCards
                editable={!isIssued}
                value={Array.isArray(v[s.key]) && v[s.key].length
                        ? v[s.key]
                        : (s.groups?.length ? s.groups : [partyGroup(2)])}
                onChange={(g)=>setV((x)=>({ ...x, [s.key]: g }))}
              />
            </div>
          )}
