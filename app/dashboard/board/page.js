...
const KIND = { owner:'مالك', partner:'شريك', board:'عضو مجلس إدارة' };
const ROLES = ['رئيس مجلس الإدارة','نائب الرئيس','عضو مجلس إدارة',
               'المدير التنفيذي','العضو المنتدب','أمين السر'];

const EMPTY = {
  employee_no:'', full_name_ar:'', full_name_en:'', nationality:'',
  id_kind:'national_id', id_number:'', id_expiry:'', mobile:'', email:'',
  person_kind:'board', board_role:'عضو مجلس إدارة', ownership_pct:'',
...
export default function Board() {
  ...
  async function load() {
    ...
    setRows(b.data || []); setRole(u.data?.role || null);
  }
  ...
  async function save(e) {
    ...
    ['seq','kind_label','years_served','job_title'].forEach((k)=>{ delete p[k]; });
    ...
    setMsg(editId ? 'حُفظت التعديلات' : 'أُضيف العضو');
  ...
  async function remove(r) {
    ...
    else { setMsg(data); load(); notifyChange('board'); }
  }
    ...
    <>
      <div className="page-head">
        <div>
          ...
          <p>{rows.length} عضواً — خارج مسير الرواتب وتقرير الموظفين</p>
        </div>
      ...
      {totalOwn > 0 && totalOwn !== 100 && (
        <div className="msg err" style={{marginBottom:14}}>
          مجموع نسب الملكية {totalOwn}٪ — راجعها إن كان المفترض ١٠٠٪
        </div>
      )}

      {open && (
        <form onSubmit={save} className="section" style={{marginTop:0}}>
          <header><h2>{editId ? 'تعديل عضو' : 'إضافة عضو'}</h2></header>
          <div style={{padding:18}}>
            <div className="form-grid">
              ...
              <div className="field">
                ...
                <input dir="ltr" value={f.employee_no || ''} onChange={set('employee_no')}
                       placeholder="BRD-001" />
              </div>
              <div className="field">
                <label>المنصب</label>
              ...
              <div className="field">
                ...
                <input type="number" step="0.01" min="0" max="100" dir="ltr"
                       value={f.ownership_pct ?? ''} onChange={set('ownership_pct')} />
              </div>
              <div className="field">
                <label>تاريخ التعيين</label>
                <input type="date" dir="ltr" value={f.appointed_at || ''}
                       onChange={set('appointed_at')} />
              </div>
              <div className="field">
      ...
      <div className="section">
        ...
        {rows.length === 0 ? (
          <div className="empty">
            <h3>لا أعضاء</h3>
            <p>أضف الملاك وأعضاء مجلس الإدارة — يبقون خارج مسير الرواتب.</p>
          </div>
        ) : (
          <div style={{overflowX:'auto'}}>
            <table>
              <thead>
                <tr><th>الاسم</th><th>الصفة</th><th>المنصب</th>
                    <th className="num">الملكية</th><th>رقم الهوية</th>
                    <th>التعيين</th><th>الجوال</th>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td style={{fontSize:12.5}}>{r.board_role || '—'}</td>
                      <td className="num">
                        {r.ownership_pct != null ? `${r.ownership_pct}%` : '—'}
                      </td>
                      <td className="mono" style={{fontSize:12.5}}>
                        {r.id_number || '—'}
                        {left != null && left <= 60 && (
                          <div><span className={`pill ${left < 0 ? 'bad' : 'warn'}`}
                                     style={{fontSize:10.5}}>
                      ...
                      <td className="mono" style={{fontSize:12.5}}>{r.mobile || '—'}</td>
                      <td>
                        {canWrite && (
                          <div className="rowsplit">
