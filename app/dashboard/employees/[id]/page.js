...
export default function EmployeePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRow(data);
      const { data:balance } = await supabase.from('v_leave_balance')
  ...
  return <>
    ...
    <div className="grid k4" style={{marginBottom:20}}>
      <div className="card"><h3>الراتب الإجمالي</h3><div className="big">{money(gross)}</div><div className="foot">ريال شهرياً</div></div>
      <div className="card"><h3>رصيد الإجازة</h3><div className="big">{bal ? bal.remaining_days : '—'}</div><div className="foot">{bal ? `من ${bal.entitled_days} يوماً` : 'لم يُنشأ رصيد بعد'}</div></div>
      <div className="card"><h3>الحالة</h3><div className="big" style={{fontSize:20,paddingTop:8}}>{STATUS_AR[row.status] || row.status}</div><div className="foot">{row.nationality || '—'}</div></div>
      <div className="card"><h3>نسبة العمولة</h3><div className="big">{(Number(row.commission_rate||0)*100).toFixed(1)}%</div><div className="foot">من ربح المشاريع التي يجلبها</div></div>
    </div>

    <div className="tabs">
      {TABS.map((item) => <button key={item.k} className={tab === item.k ? 'on' : ''} onClick={()=>setTab(item.k)}>{item.label}</button>)}
    </div>
