...
export default function Advances(){
  ...
  function startEdit(r){setSelected(null);setDecisionTarget(null);setDisbursementTarget(null);setEditId(r.id);setForm({employee_id:r.employee_id,amount:r.amount,installments:r.installments,first_deduction_month:r.first_deduction_month||'',reason:r.reason||''});setOpen(true);setErr('');setMsg('');}
  function startNew(){setSelected(null);setDecisionTarget(null);setDisbursementTarget(null);setEditId(null);setForm({...EMPTY});setOpen(true);setErr('');setMsg('');}
  function startDecision(r){setSelected(null);setOpen(false);setEditId(null);setDisbursementTarget(null);setDecisionTarget(r);setErr('');setMsg('');}
  function startDisbursement(r){setSelected(null);setOpen(false);setEditId(null);setDecisionTarget(null);setDisbursementTarget(r);setDisbursementDate(operationalDate());setDisbursementRef('');setErr('');setMsg('');}
  function openRecord(r){setOpen(false);setDecisionTarget(null);setDisbursementTarget(null);setSelected(r);setErr('');setMsg('');}
  ...
  async function cancel(r){const reason=window.prompt('سبب إلغاء السلفة:');if(reason===null)return;setErr('');setMsg('');const {error}=await supabase.rpc('cancel_advance',{p_id:r.id,p_reason:reason});if(error){setErr(error.message);return;}setSelected(null);setMsg('ألغيت السلفة');load();}
  async function remove(r){if(!window.confirm('حذف هذا الطلب نهائيًا؟'))return;setErr('');setMsg('');const {error}=await supabase.from('advances').delete().eq('id',r.id);if(error){setErr('تعذّر الحذف: '+error.message);return;}setSelected(null);setMsg('حذف الطلب');load();}
  ...
  const selectedStage=selected?nextStage('advance',selected.status):null;
  ...
  return <ConstitutionPage>
    <PageHeader eyebrow="المالية" title="السلف والمديونيات" description="السجل للمتابعة. افتح طلبًا واحدًا فقط لتنفيذ الإجراء المناسب لحالته." actions={<button className="btn" onClick={startNew}>+ طلب سلفة</button>}/>
    ...
    <Notice>المبدأ التشغيلي: الجدول للعرض فقط. الاعتماد والصرف مرحلتان منفصلتان، والأقساط تبدأ فقط بعد تسجيل خروج المبلغ فعليًا.</Notice>
    ...
    {selected&&<EntrySurface title={`طلب سلفة — ${selected.employees?.full_name_ar||'غير محدد'}`} description={selectedStage?`المطلوب التالي: ${STAGE_AR[selectedStage]}`:selected.status==='ceo_approved'&&!selected.disbursed_at?'تم الاعتماد — بانتظار تسجيل الصرف':'لا يوجد إجراء اعتماد تالٍ'}>
      <div style={{padding:22}}>
        <div className="form-grid">
          <div className="field"><label>المبلغ</label><strong>{money(selected.amount)} ريال</strong></div>
          <div className="field"><label>عدد الأقساط</label><strong>{selected.installments}</strong></div>
          <div className="field"><label>الحالة</label><span className={`pill ${STATUS_CLASS[selected.status]}`}>{STATUS_AR[selected.status]}</span></div>
          <div className="field"><label>المسار الحالي</label><strong>{selectedStage?STAGE_AR[selectedStage]:selected.status==='ceo_approved'&&!selected.disbursed_at?'تسجيل الصرف':'مكتملة'}</strong></div>
          <div className="field span2"><label>السبب</label><div>{selected.reason||'—'}</div></div>
          {selected.disbursed_at&&<div className="field"><label>تاريخ الصرف</label><strong>{dateAr(selected.disbursed_at)}</strong></div>}
        </div>
        <Toolbar>
          ...
          {selected.status==='ceo_approved'&&!selected.disbursed_at&&<button className="btn" onClick={()=>startDisbursement(selected)}>تسجيل الصرف</button>}
          ...
          {canEdit(selected)&&<button className="btn ghost" onClick={()=>remove(selected)}>حذف</button>}
    ...
    {open&&<EntrySurface title={editId?'تعديل طلب سلفة':'طلب سلفة'} description="أدخل بيانات الطلب فقط؛ بعد الحفظ يعود الطلب للسجل.">
