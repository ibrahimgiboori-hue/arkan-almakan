...
import { STATUS_AR, STATUS_CLASS, LEAVE_AR } from '@/lib/requests';
...
const LEAVE_FLOW_AR={
  draft:'مسودة / معاد للتعديل',
  submitted:'بانتظار مراجعة الموارد البشرية',
  hr_reviewed:'بانتظار الاعتماد النهائي',
  accountant_approved:'بانتظار الاعتماد النهائي',
  ceo_approved:'مكتملة',
  rejected:'مرفوضة',
  cancelled:'ملغاة',
};
...
export default function Leaves(){
  ...
  const canEdit=(r)=>r.record_source!=='historical_paper'&&r.status==='draft';
  const canCancel=(r)=>r.record_source!=='historical_paper'&&!['cancelled','rejected'].includes(r.status);

  function closeForms(){setOpen(false);setHistoryOpen(false);setEditId(null);}
  function startEdit(r){setSelected(null);setHistoryOpen(false);setEditId(r.id);setForm({employee_id:r.employee_id,leave_kind:r.leave_kind,start_date:r.start_date,end_date:r.end_date,reason:r.reason||''});setOpen(true);setErr('');setMsg('');}
  function startNew(){setSelected(null);setHistoryOpen(false);setEditId(null);setForm({...EMPTY});setOpen(true);setErr('');setMsg('');}
  function startHistory(){setSelected(null);setOpen(false);setEditId(null);setHistory({...HISTORY_EMPTY});setHistoryOpen(true);setErr('');setMsg('');}
  function openRecord(r){setOpen(false);setHistoryOpen(false);setSelected(r);setErr('');setMsg('');}
  ...
  async function submit(e){
    ...
    const saveResult=editId
      ?await supabase.from('leave_requests').update(payload).eq('id',editId).select('id').single()
      :await supabase.from('leave_requests').insert({...payload,status:'draft'}).select('id').single();
    if(saveResult.error){setSaving(false);setErr('تعذر الحفظ: '+saveResult.error.message);return;}

    const requestId=saveResult.data?.id||editId;
    const employeeName=emps.find((employee)=>employee.id===form.employee_id)?.full_name_ar||'موظف';
    const {error:approvalError}=await supabase.rpc('fn_universal_submit_for_approval',{
      p_capability_key:'hr.leaves.create',
      p_source_table:'leave_requests',
      p_source_id:requestId,
      p_source_label:`طلب إجازة — ${employeeName}`,
      p_note:form.reason?.trim()||null,
    });
    setSaving(false);
    if(approvalError){
      setEditId(requestId);
      setErr('تم حفظ الطلب كمسودة، لكن تعذر إرساله إلى مركز الاعتمادات: '+approvalError.message);
      await load();
      return;
    }

    setMsg(editId?'تم حفظ التعديلات وإعادة إرسال الطلب إلى مركز الاعتمادات':'تم تسجيل طلب الإجازة وإرساله إلى مركز الاعتمادات');
    setForm({...EMPTY});setEditId(null);setOpen(false);await load();
  }
  ...
  async function cancel(r){const reason=window.prompt(`سبب إلغاء إجازة ${r.employees?.full_name_ar||''}:`);if(reason===null)return;setErr('');setMsg('');const {error}=await supabase.rpc('cancel_leave',{p_id:r.id,p_reason:reason});if(error){setErr(error.message);return;}setSelected(null);setMsg('تم إلغاء الطلب وإغلاق مسار اعتماده');load();}
  async function remove(r){if(!window.confirm('حذف هذا السجل نهائياً؟'))return;setErr('');setMsg('');const {error}=await supabase.from('leave_requests').delete().eq('id',r.id);if(error){setErr('تعذر الحذف: '+error.message);return;}setSelected(null);setMsg('تم حذف السجل');load();}
  ...
  return <ConstitutionPage>
    <PageHeader eyebrow="الموارد البشرية" title="الإجازات" description="سجل الإجازات ومتابعتها؛ القرارات الرسمية تتخذ من مركز الاعتمادات الموحد." actions={<Toolbar><Link className="btn ghost" href="/dashboard/approvals">مركز الاعتمادات</Link><button className="btn ghost" onClick={startHistory}>+ حركة تاريخية</button><button className="btn" onClick={startNew}>+ طلب إجازة</button></Toolbar>}/>
    ...
    <Notice>طلب الإجازة يُسجل هنا ثم يدخل تلقائيًا إلى مسار الاعتماد الموحد. هذه الصفحة للبيانات والمتابعة؛ المراجعة والاعتماد والرفض والإرجاع تتم من «مركز الاعتمادات». الملفات الورقية القديمة لا تمر بمسار اعتماد جديد.</Notice>
    {err&&<Notice tone="error">{err}</Notice>}{msg&&<Notice tone="success">{msg}</Notice>}

    {selected&&<EntrySurface title={`إجازة — ${selected.employees?.full_name_ar||'غير محدد'}`} description={selected.record_source==='historical_paper'?'حركة تاريخية مكتملة':LEAVE_FLOW_AR[selected.status]||'طلب إجازة'}>
      <div style={{padding:22}}>
        <div className="form-grid">
          <div className="field"><label>النوع</label><strong>{LEAVE_AR[selected.leave_kind]||selected.leave_kind}</strong></div>
          <div className="field"><label>الفترة</label><strong>{dateAr(selected.start_date)} — {dateAr(selected.end_date)}</strong></div>
          <div className="field"><label>الأيام</label><strong>{selected.days_count}</strong></div>
          <div className="field"><label>الحالة</label><span className={`pill ${STATUS_CLASS[selected.status]}`}>{selected.record_source==='historical_paper'?'منفذة تاريخياً':STATUS_AR[selected.status]}</span></div>
          <div className="field span2"><label>السبب / الملاحظة</label><div>{selected.reason||'—'}</div></div>
        </div>
        <Toolbar>
          <Link className="btn ghost" href={`/print/leave/${selected.id}`} target="_blank">طباعة</Link>
          {selected.record_source!=='historical_paper'&&['submitted','hr_reviewed','accountant_approved'].includes(selected.status)&&<Link className="btn" href="/dashboard/approvals">فتح مركز الاعتمادات</Link>}
          {canEdit(selected)&&<button className="btn ghost" onClick={()=>startEdit(selected)}>تعديل وإعادة إرسال</button>}
          {canCancel(selected)&&<button className="btn ghost" onClick={()=>cancel(selected)}>إلغاء الطلب</button>}
          {(canEdit(selected)||selected.record_source==='historical_paper')&&<button className="btn ghost" onClick={()=>remove(selected)}>حذف</button>}
    ...
    {open&&<EntrySurface title={editId?'تعديل طلب إجازة':'طلب إجازة'} description="بعد الحفظ يُرسل الطلب مباشرة إلى مركز الاعتمادات؛ إذا تعذر الإرسال يبقى محفوظًا كمسودة قابلة للتصحيح.">
      ...
      </div><Toolbar><button className="btn" type="submit" disabled={saving}>{saving?'جارٍ الحفظ والإرسال…':editId?'حفظ وإعادة إرسال':'تسجيل وإرسال'}</button><button className="btn ghost" type="button" onClick={closeForms} disabled={saving}>إلغاء</button></Toolbar></form>
    </EntrySurface>}

    <Section title="حركات وطلبات الإجازة" description={`${rows.length} حركة مسجلة`}>
      {rows.length===0?<EmptyState title="لا توجد حركات" description="سجل طلباً جديداً أو أدخل حركة تاريخية."/>:<TableFrame><table><thead><tr><th>الموظف</th><th>المصدر</th><th>النوع</th><th>من</th><th>إلى</th><th className="num">الأيام</th><th>الحالة</th><th>المسار الحالي</th><th>فتح</th></tr></thead><tbody>{rows.map((r)=>{const flow=r.record_source==='historical_paper'?'مكتملة تاريخيًا':LEAVE_FLOW_AR[r.status]||'—';return <tr key={r.id}><td><strong>{r.employees?.full_name_ar||'غير محدد'}</strong></td><td>{r.record_source==='historical_paper'?'ملف ورقي قديم':'طلب حالي'}</td><td>{LEAVE_AR[r.leave_kind]}</td><td className="mono">{dateAr(r.start_date)}</td><td className="mono">{dateAr(r.end_date)}</td><td className="num">{r.days_count}</td><td><span className={`pill ${STATUS_CLASS[r.status]}`}>{r.record_source==='historical_paper'?'منفذة تاريخياً':STATUS_AR[r.status]}</span></td><td>{flow}</td><td><button className="btn ghost" onClick={()=>openRecord(r)}>فتح</button></td></tr>;})}</tbody></table></TableFrame>}
    </Section>
