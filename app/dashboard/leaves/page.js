...
export default function Leaves(){
  ...
  const [selected,setSelected]=useState(null); const [decisionTarget,setDecisionTarget]=useState(null); const [decisionBusy,setDecisionBusy]=useState(false);
  ...
  function startEdit(r){setSelected(null);setDecisionTarget(null);setHistoryOpen(false);setEditId(r.id);setForm({employee_id:r.employee_id,leave_kind:r.leave_kind,start_date:r.start_date,end_date:r.end_date,reason:r.reason||''});setOpen(true);setErr('');setMsg('');}
  function startNew(){setSelected(null);setDecisionTarget(null);setHistoryOpen(false);setEditId(null);setForm({...EMPTY});setOpen(true);setErr('');setMsg('');}
  function startHistory(){setSelected(null);setDecisionTarget(null);setOpen(false);setEditId(null);setHistory({...HISTORY_EMPTY});setHistoryOpen(true);setErr('');setMsg('');}
  function startDecision(r){setSelected(null);setOpen(false);setHistoryOpen(false);setEditId(null);setDecisionTarget(r);setErr('');setMsg('');}
  function openRecord(r){setOpen(false);setHistoryOpen(false);setDecisionTarget(null);setSelected(r);setErr('');setMsg('');}
  ...
  async function cancel(r){const reason=window.prompt(`سبب إلغاء إجازة ${r.employees?.full_name_ar||''}:`);if(reason===null)return;setErr('');setMsg('');const {error}=await supabase.rpc('cancel_leave',{p_id:r.id,p_reason:reason});if(error){setErr(error.message);return;}setSelected(null);setMsg('تم إلغاء الطلب');load();}
  ...
  const selectedStage=selected&&selected.record_source!=='historical_paper'?nextStage('leave',selected.status):null;

  return <ConstitutionPage>
    <PageHeader eyebrow="الموارد البشرية" title="الإجازات" description="السجل للمتابعة. افتح معاملة واحدة فقط عند الحاجة إلى إجراء عليها." actions={<Toolbar><button className="btn ghost" onClick={startHistory}>+ حركة تاريخية</button><button className="btn" onClick={startNew}>+ طلب إجازة</button></Toolbar>}/>
    ...
    <Notice>المبدأ التشغيلي: الجدول للعرض فقط. زر «فتح» يدخل المعاملة، ومن داخلها يظهر الإجراء المناسب لحالتها. الملفات الورقية القديمة لا تمر بمسار اعتماد جديد.</Notice>
    ...
    {selected&&<EntrySurface title={`إجازة — ${selected.employees?.full_name_ar||'غير محدد'}`} description={selected.record_source==='historical_paper'?'حركة تاريخية مكتملة':selectedStage?`المطلوب التالي: ${STAGE_AR[selectedStage]}`:'لا يوجد إجراء اعتماد تالٍ'}>
      <div style={{padding:22}}>
        ...
        <Toolbar>
          ...
          {selectedStage&&!['cancelled','rejected'].includes(selected.status)&&<button className="btn" onClick={()=>startDecision(selected)}>تسجيل الإجراء</button>}
          {canEdit(selected)&&<button className="btn ghost" onClick={()=>startEdit(selected)}>تعديل</button>}
    ...
    {open&&<EntrySurface title={editId?'تعديل طلب إجازة':'طلب إجازة'} description="أدخل بيانات الطلب فقط؛ بعد الحفظ يعود الطلب للسجل.">
    ...
    <Section title="حركات وطلبات الإجازة" description={`${rows.length} حركة مسجلة`}>
      {rows.length===0?<EmptyState title="لا توجد حركات" description="سجل طلباً جديداً أو أدخل حركة تاريخية."/>:<TableFrame><table><thead><tr><th>الموظف</th><th>المصدر</th><th>النوع</th><th>من</th><th>إلى</th><th className="num">الأيام</th><th>الحالة</th><th>المسار الحالي</th><th>فتح</th></tr></thead><tbody>{rows.map((r)=>{const stage=r.record_source==='historical_paper'?null:nextStage('leave',r.status);return <tr key={r.id}><td><strong>{r.employees?.full_name_ar||'غير محدد'}</strong></td><td>{r.record_source==='historical_paper'?'ملف ورقي قديم':'طلب حالي'}</td><td>{LEAVE_AR[r.leave_kind]}</td><td className="mono">{dateAr(r.start_date)}</td><td className="mono">{dateAr(r.end_date)}</td><td className="num">{r.days_count}</td><td><span className={`pill ${STATUS_CLASS[r.status]}`}>{r.record_source==='historical_paper'?'منفذة تاريخياً':STATUS_AR[r.status]}</span></td><td>{stage?STAGE_AR[stage]:'مكتملة'}</td><td><button className="btn ghost" onClick={()=>openRecord(r)}>فتح</button></td></tr>;})}</tbody></table></TableFrame>}
    </Section>
