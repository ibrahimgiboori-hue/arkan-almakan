...
const SECTIONS = [
  ...
  ['show_letterhead','خلفية الترويسة'],
];
...
export default function QuoteEditor() {
  ...
  async function move(lineId, dir) {
    ...
    // تبديل مؤقت لتجنب تعارض الفهرس الفريد
                ...
                {numbered.map((l) => l.kind === 'title' ? (
                  <tr key={l.id} style={{background:'var(--rose-wash)'}}>
                    ...
                    <td colSpan={1 + (q.show_unit?1:0) + (q.show_qty?1:0) + (q.show_unit_price?1:0)}>
                      <input value={l.description_ar || ''} placeholder="عنوان القسم"
                             onChange={(e)=>updLine(l.id,{description_ar:e.target.value})}
                  ...
                  <tr key={l.id}>
                    ...
                    <td>
                      ...
                      {q.show_en_desc && (
                        <input dir="ltr" value={l.description_en || ''} placeholder="English description"
                               onChange={(e)=>updLine(l.id,{description_en:e.target.value})}
                               style={{width:'100%',border:'1px solid var(--hair)',fontSize:12.5,
                                       padding:'3px 6px',marginTop:3,fontFamily:'inherit'}} />
                      )}
                      <div className="rowsplit" style={{marginTop:4}}>
                        {items.length > 0 && (
                          <select defaultValue="" onChange={(e)=>pickItem(l.id, e.target.value)}
                                  style={{fontSize:12,padding:'2px 4px',maxWidth:180}}>
                            <option value="">من دليل البنود…</option>
                            {items.map((w)=>(
                              <option key={w.id} value={w.id}>{w.description_ar?.slice(0,42)}</option>
                            ))}
                          </select>
                        )}
                        <button className="btn ghost" style={{padding:'2px 7px',fontSize:11.5}}
                                onClick={()=>saveToLibrary(l)}>حفظ في الدليل</button>
                      </div>
                    </td>
                    {q.show_unit && (
                      <td><input value={l.unit || ''} onChange={(e)=>updLine(l.id,{unit:e.target.value})}
                                 style={{width:'100%',border:'1px solid var(--hair)',fontSize:13,padding:'4px'}} /></td>
                    )}
                    {q.show_qty && (
                      <td><input type="number" step="any" dir="ltr" value={l.qty ?? ''}
                                 onChange={(e)=>updLine(l.id,{qty:Number(e.target.value||0)})}
                                 style={{width:'100%',border:'1px solid var(--hair)',fontSize:13,padding:'4px',textAlign:'left'}} /></td>
                    )}
                    {q.show_unit_price && (
                      <td><input type="number" step="0.01" dir="ltr" value={l.unit_price ?? ''}
        ...
        <div className="section" style={{marginTop:0,padding:18}}>
          <div className="form-grid">
            <div className="field span2">
              <label>العميل *</label>
              <input value={q.client_name || ''} onChange={(e)=>setQ({...q,client_name:e.target.value})}
                     onBlur={(e)=>patch({client_name:e.target.value})} />
            </div>
            <div className="field">
              <label>جهة الاتصال</label>
              <input value={q.client_contact || ''} onChange={(e)=>setQ({...q,client_contact:e.target.value})}
                     onBlur={(e)=>patch({client_contact:e.target.value})} />
            </div>
            <div className="field span2">
              <label>المرجع وتفاصيل المشروع</label>
              <input value={q.project_ref || ''} onChange={(e)=>setQ({...q,project_ref:e.target.value})}
                     onBlur={(e)=>patch({project_ref:e.target.value})} />
            </div>
            <div className="field">
              <label>الموقع</label>
              <input value={q.site_location || ''} onChange={(e)=>setQ({...q,site_location:e.target.value})}
                     onBlur={(e)=>patch({site_location:e.target.value})} />
            </div>
            <div className="field">
              <label>التاريخ</label>
              <input type="date" dir="ltr" value={q.quote_date || ''}
            ...
            <div className="field">
              <label>الحالة</label>
              <select value={q.status} onChange={(e)=>patch({status:e.target.value})}>
                {Object.entries(QSTATUS_AR).map(([k,v])=><option key={k} value={k}>{v}</option>)}
              </select>
            </div>
            <div className="field">
              <label>معالجة الضريبة</label>
              <select value={q.vat_mode} onChange={(e)=>patch({vat_mode:e.target.value})}>
                <option value="exclusive">تُضاف على الأسعار (١٥٪)</option>
                <option value="inclusive">الأسعار شاملة الضريبة</option>
                <option value="none">بلا ضريبة</option>
              </select>
            </div>
            <div className="field">
              <label>خصم نسبة (٠.٠٥ = ٥٪)</label>
              <input type="number" step="0.01" dir="ltr" value={q.discount_pct}
                     onChange={(e)=>setQ({...q,discount_pct:e.target.value})}
                     onBlur={(e)=>patch({discount_pct:Number(e.target.value||0)})} />
            </div>
            <div className="field">
              <label>خصم مبلغ ثابت</label>
              <input type="number" step="0.01" dir="ltr" value={q.discount_amount}
                     onChange={(e)=>setQ({...q,discount_amount:e.target.value})}
                     onBlur={(e)=>patch({discount_amount:Number(e.target.value||0)})} />
            </div>
            <div className="field span2">
              <label>عنوان مخصص للمستند</label>
              <input value={q.title_override || ''} placeholder="اتركه فارغاً للعنوان الافتراضي"
                     onChange={(e)=>setQ({...q,title_override:e.target.value})}
                     onBlur={(e)=>patch({title_override:e.target.value})} />
            </div>
          </div>
        </div>
      )}
            ...
            <div style={{padding:'12px 18px'}}>
              {TOGGLES.map(([k,label]) => (
                <label key={k} style={{display:'flex',alignItems:'center',gap:9,padding:'7px 0',cursor:'pointer'}}>
                  <input type="checkbox" checked={!!q[k]} onChange={(e)=>patch({[k]:e.target.checked})} />
                  <span style={{fontSize:14}}>{label}</span>
                </label>
              ))}
              <div className="hint" style={{marginTop:8}}>
                أخفِ الكمية والإجمالي فيخرج العرض مقطوعيات — وأظهرهما فيخرج جدول كميات
              </div>
            </div>
          </div>
          <div className="section" style={{marginTop:0}}>
            <header><h2>أقسام المستند</h2></header>
            <div style={{padding:'12px 18px'}}>
      ...
      {tab === 'pay' && (
        <>
          <div className="rowsplit" style={{marginBottom:12}}>
            ...
            <span className="spacer" />
            <span style={{fontSize:13,color: payPctSum === 100 ? 'var(--ok)' : 'var(--warn)'}}>
              مجموع النسب: {payPctSum}% {payPctSum === 100 ? '✓' : '— يجب أن يساوي ١٠٠٪'}
            </span>
          </div>
          <div className="section" style={{marginTop:0}}>
            <table>
              ...
              <tbody>
                {pays.map((p) => (
                  <tr key={p.id}>
                    <td><input value={p.label || ''} onChange={(e)=>updPay(p.id,{label:e.target.value})}
                    ...
                    <td><input value={p.trigger_note || ''} placeholder="مثال: عند توقيع العقد"
                               ...
                               style={{width:'100%',border:'1px solid var(--hair)',padding:'4px',fontSize:13.5,fontFamily:'inherit'}} /></td>
                    <td><button className="btn ghost" style={{padding:'3px 9px',fontSize:12}}
                                onClick={()=>delPay(p.id)}>حذف</button></td>
                  </tr>
                ))}
                {pays.length === 0 && (
                  <tr><td colSpan={5}>
                    <div className="empty"><h3>لا دفعات</h3>
                      <p>أضف الدفعات المقترحة، والمبالغ تُحسب من المجموع تلقائياً.</p></div>
                  </td></tr>
                )}
              </tbody>
            </table>
        ...
        <div className="section" style={{marginTop:0,padding:18}}>
          <div className="field">
            ...
            <textarea rows="3" value={q.intro_text || ''}
                      ...
                      onBlur={(e)=>patch({intro_text:e.target.value})} />
          </div>
          <div className="field">
            <label>الشروط والأحكام — كل شرط في سطر</label>
            <textarea rows="8" value={q.terms_text || ''}
                      onChange={(e)=>setQ({...q,terms_text:e.target.value})}
                      onBlur={(e)=>patch({terms_text:e.target.value})} />
            <span className="hint">يُرقّمها النظام تلقائياً ١، ٢، ٣ … فلا تكتب الأرقام</span>
          </div>
          <div className="field">
            <label>النص الختامي (بعد الجدول)</label>
            <textarea rows="2" value={q.closing_text || ''}
                      onChange={(e)=>setQ({...q,closing_text:e.target.value})}
                      onBlur={(e)=>patch({closing_text:e.target.value})} />
          </div>
        </div>
      )}
    </>
