...
function editablePayload(row){
  return {
    ...
    basic_salary:n(row.basic_salary),
    housing_allowance:n(row.housing_allowance),
    transport_allowance:n(row.transport_allowance),
    other_allowance:n(row.other_allowance),
    overtime_amount:n(row.overtime_amount),
    commission_amount:n(row.commission_amount),
    absence_days:n(row.absence_days),
    absence_deduction:n(row.absence_deduction),
    advance_deduction:n(row.advance_deduction),
    penalty_deduction:n(row.penalty_deduction),
    gosi_deduction:n(row.gosi_deduction),
    other_deduction:n(row.other_deduction),
...
function newLine(runId,employee){
  return {
    ...
    basic_salary:n(employee.basic_salary),
    housing_allowance:n(employee.housing_allowance),
    transport_allowance:n(employee.transport_allowance),
    other_allowance:n(employee.other_allowance),
    overtime_amount:0,
    commission_amount:0,
    absence_days:0,
    absence_deduction:0,
...
export default function PayrollOperationalPage(){
  const [auth,setAuth]=useState({loading:true,allowed:false,canEdit:false,error:''});
  ...
  useEffect(()=>{let alive=true;(async()=>{
    ...
    if(!session){if(alive)setAuth({loading:false,allowed:false,canEdit:false,error:'يلزم تسجيل الدخول.'});return;}
    ...
    if(alive)setAuth({loading:false,allowed,canEdit,error:allowed?'':'هذا القسم خارج الصلاحيات الممنوحة لهذا الحساب.'});
  })();return()=>{alive=false;};},[]);
  ...
  async function load(preferredRunId=null){
    ...
    const nextRuns=runsQ.data||[];const nextEmployees=employeesQ.data||[];
    setRuns(nextRuns);setEmployees(nextEmployees);
    const chosen=nextRuns.find(r=>r.id===preferredRunId)||nextRuns.find(r=>r.id===run?.id)||nextRuns[0]||null;
    setRun(chosen);
    if(chosen){
      const linesQ=await supabase.from('payroll_lines').select('*').eq('run_id',chosen.id).order('employee_id');
      if(linesQ.error)setError(linesQ.error.message);
  ...
  async function selectRun(id){
    const chosen=runs.find(r=>r.id===id)||null;setRun(chosen);setLoading(true);setError('');
    if(!chosen){setLines([]);setLoading(false);return;}
    const q=await supabase.from('payroll_lines').select('*').eq('run_id',id).order('employee_id');
    if(q.error)setError(q.error.message);setLines(q.data||[]);setLoading(false);
  }
  ...
  async function ensureEmployeeLines(runId){
    const existingQ=await supabase.from('payroll_lines').select('employee_id').eq('run_id',runId);
    ...
    if(missing.length){
      const insertQ=await supabase.from('payroll_lines').insert(missing);
  ...
  async function createOrOpenMonth(){
    if(!auth.canEdit||!month)return;
    setBusy(true);setError('');setMessage('');
    try{
      const runMonth=`${month}-01`;
      const existingQ=await supabase.from('payroll_runs').select('*').eq('run_month',runMonth).maybeSingle();
      if(existingQ.error)throw existingQ.error;
      let current=existingQ.data;
      if(!current){
        const createQ=await supabase.from('payroll_runs').insert({run_month:runMonth,status:'draft'}).select('*').single();
        if(createQ.error)throw createQ.error;
        current=createQ.data;
      }
      const added=await ensureEmployeeLines(current.id);
      setMessage(existingQ.data?`تم فتح دورة الشهر${added?` وإضافة ${added} موظف غير موجود`:''}.`:`تم إنشاء مسودة الرواتب وإضافة ${added} موظف.`);
  ...
  async function saveLines(){
    if(!auth.canEdit||!run||run.status!=='draft')return;
    setBusy(true);setError('');setMessage('');
    try{
      const payload=lines.map(editablePayload);
      if(payload.length){
        const saveQ=await supabase.from('payroll_lines').upsert(payload,{onConflict:'id'});
        if(saveQ.error)throw saveQ.error;
      }
      const computed=lines.map(calculated);
      const totals=computed.reduce((a,r)=>({gross:a.gross+n(r.gross_pay),ded:a.ded+n(r.total_deductions),net:a.net+n(r.net_pay)}),{gross:0,ded:0,net:0});
      const runQ=await supabase.from('payroll_runs').update({total_gross:totals.gross,total_deductions:totals.ded,total_net:totals.net}).eq('id',run.id);
      if(runQ.error)throw runQ.error;
      setMessage('تم حفظ دورة الرواتب. الإجمالي والصافي محسوبان تلقائيًا من قاعدة البيانات.');
      await load(run.id);
    }catch(e){setError(e?.message||'تعذر حفظ دورة الرواتب.');}
    setBusy(false);
  }
  ...
  return <ConstitutionPage>
    <Section title="إعداد الرواتب" description="إنشاء دورة شهرية وتعبئة رواتب الموظفين. الإجمالي والخصومات والصافي أعمدة محسوبة تلقائيًا في قاعدة البيانات ولا يتم إدخالها يدويًا.">
      <div style={{display:'grid',gap:14}}>
        ...
        {run?<RawGridFooter actions={<button className="btn" type="button" disabled={busy||locked} onClick={saveLines}>حفظ دورة الرواتب</button>} summary={<strong>الإجمالي {money(totals.gross)} · الخصومات {money(totals.ded)} · الصافي {money(totals.net)}</strong>}/>:null}
      </div>
    </Section>
  </ConstitutionPage>;
}
