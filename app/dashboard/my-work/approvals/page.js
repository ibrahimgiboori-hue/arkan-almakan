...
function money(value){
  ...
  return n?`${n.toLocaleString('ar-SA',{maximumFractionDigits:2})} ر.س`:'—';
}
...
export default function MyApprovalsPage(){
  ...
  const load=useCallback(async()=>{
    ...
    const list=data||[];
    setRows(list);
    setSelectedId(current=>current&&list.some(x=>x.workflow_id===current)?current:(list[0]?.workflow_id||''));
  },[]);
  ...
  useEffect(()=>{
    ...
    let alive=true;
    setDetail(null);setError('');setNote('');
    supabase.rpc('fn_approval_get',{p_workflow_id:selectedId}).then(({data,error:rpcError})=>{
      if(!alive)return;
      if(rpcError){setError(rpcError.message||'تعذر قراءة تفاصيل المعاملة.');setDetail(null);}
      else setDetail(data||null);
    });
    return()=>{alive=false;};
  },[selectedId]);

  const selected=useMemo(()=>rows?.find(x=>x.workflow_id===selectedId)||null,[rows,selectedId]);
  ...
  async function decide(decision){
    ...
    if(rpcError)setError(rpcError.message||'تعذر تنفيذ القرار.');
      ...
      setMessage(decision==='approve'?'تم اعتماد المعاملة.':decision==='return'?'تم إرجاع المعاملة للتعديل.':'تم رفض المعاملة.');
      setNote('');
      await load();
    }
    setBusy('');
  ...
  if(rows===null)return <ConstitutionPage><EmptyState title="جارٍ تحميل الاعتمادات" description="يتم جمع المعاملات المسندة إليك أو إلى صلاحياتك الحالية."/></ConstitutionPage>;
  ...
  return <ConstitutionPage>
    <PageHeader eyebrow="APPROVALS" title="اعتمادات بانتظاري" description="المعاملات النظامية التي تحتاج قرارك الآن. مصدر الحقيقة هو محرك الاعتماد الموحد، وليس مهام تواصل العمل."/>
    {error?<Notice tone="warning">{error}</Notice>:null}
    {message?<Notice tone="success">{message}</Notice>:null}

    <div className={styles.shell}>
      <Section title="الوارد للاعتماد" description={`${rows.length} معاملة بانتظار قرارك`}>
        {rows.length===0?<EmptyState title="لا توجد اعتمادات بانتظارك" description="ستظهر هنا التايم شيت والمستخلصات وبقية العمليات فور إسناد مرحلة اعتماد إليك أو إلى صلاحية تملكها."/>:<div className={styles.list}>
          {rows.map(row=><button type="button" key={row.workflow_id} className={`${styles.item} ${row.workflow_id===selectedId?styles.active:''}`} onClick={()=>setSelectedId(row.workflow_id)}>
      ...
      <div className={styles.detail}>
        {!selected?<EmptyState title="اختر معاملة" description="اختر معاملة من القائمة لعرض مسارها واتخاذ القرار."/>:!workflow?<EmptyState title="جارٍ قراءة المعاملة" description="يتم تحميل تفاصيل النسخة الحالية وسجل القرارات."/>:<Section title={workflow.source_label||selected.label_ar||'معاملة اعتماد'} description={`${workflow.workflow_no||'—'} · النسخة ${workflow.version_no||1}`}>
          ...
          <div className={styles.block}>
            <h3>مسار الاعتماد</h3>
            {steps.length===0?<div className={styles.muted}>لا توجد خطوات مسجلة.</div>:<div className={styles.timeline}>{steps.map(step=><div className={styles.event} key={step.id}>
          ...
          {events.length?<div className={styles.block}>
            <h3>سجل الحركة</h3>
            <div className={styles.timeline}>{events.map((event,index)=><div className={styles.event} key={`${event.created_at}-${index}`}>
          ...
          {detail?.can_act&&workflow.status==='pending'?<div className={styles.block}>
            <h3>قرارك</h3>
            <label className={styles.field}>الملاحظة / التبرير<textarea value={note} onChange={e=>setNote(e.target.value)} rows={4} maxLength={2000} placeholder="التبرير إلزامي عند الإرجاع أو الرفض"/></label>
          ...
          </div>:<div className={styles.block}><div className={styles.muted}>هذه المعاملة متاحة للمتابعة فقط أو لم تعد بانتظار قرارك.</div></div>}
        </Section>}
      </div>
    </div>
  </ConstitutionPage>;
