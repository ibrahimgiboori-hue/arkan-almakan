...
export default function TodayPage(){
  ...
  const view=useMemo(()=>{if(!state)return null;const today=dateKey();const active=state.tasks.filter(t=>!CLOSED.has(t.status));const overdue=active.filter(t=>t.due_at&&dateKey(t.due_at)<today);const dueToday=active.filter(t=>t.due_at&&dateKey(t.due_at)===today);const incoming=active.filter(t=>t.assignee_user_id===state.uid&&t.creator_user_id!==state.uid);const newIncoming=incoming.filter(t=>['new','received'].includes(t.status));const mine=active.filter(t=>t.task_type==='personal_task'||t.assignee_user_id===state.uid);const todayRows=[...new Map([...overdue,...dueToday,...newIncoming,...mine.filter(t=>['in_progress','waiting'].includes(t.status))].map(t=>[t.id,t])).values()].slice(0,8);
    const projectCaps=state.capabilities.filter(c=>c.module_key==='projects');const projectsPortal=projectCaps.some(c=>c.source_key==='projects_full_access'&&c.scope_type==='all')||projectCaps.some(c=>c.scope_type==='all');const projectRows=state.projects.map(project=>{const caps=projectCaps.filter(c=>c.scope_type==='all'||(c.scope_type==='project'&&c.scope_key===project.id));const keys=new Set(caps.map(c=>c.capability_key));const projectFull=projectsPortal||caps.some(c=>c.source_key==='projects_full_access')||keys.has('projects.overview.view')||keys.has('projects.projects.view');return{...project,projectFull,attendance:projectFull||keys.has('projects.timesheets.view'),expenses:projectFull||keys.has('projects.expenses.view')};});
  ...
  if(!state||!view)return <ConstitutionPage><EmptyState title="جارٍ تجهيز يومك" description="يتم جمع ما يحتاج انتباهك وأعمالك ومراسلاتك والتنبيهات ونطاق العمل المسموح."/></ConstitutionPage>;
  const empName=state.employee?.full_name_ar||'المستخدم';
  return <ConstitutionPage>
    <PageHeader eyebrow="TODAY" title={`اليوم — ${empName}`} description="شاشتك الشخصية الواحدة: ما يحتاج انتباهك الآن، ثم أعمالك ومراسلاتك ومحادثاتك ومرفقاتك في نفس الصفحة." actions={<Link className="btn" href="/dashboard/workspace">منصة الأعمال ←</Link>}/>
    {err&&<Notice tone="warning">{err}</Notice>}

    <Section title="ما يحتاج انتباهك الآن"><div className="grid k4"><div className="card"><h3>وارد جديد</h3><div className="big">{view.newIncoming.length}</div><div className="foot">مهام ومراسلات من الآخرين</div></div><div className="card"><h3>متأخر</h3><div className="big">{view.overdue.length}</div><div className="foot">تجاوز موعده ولم يُغلق</div></div><div className="card"><h3>موعد اليوم</h3><div className="big">{view.dueToday.length}</div><div className="foot">مطلوب التعامل معه اليوم</div></div><div className="card"><h3>تنبيهات جديدة</h3><div className="big">{view.unread.length}</div><div className="foot">تنبيهات شخصية غير مقروءة</div></div></div></Section>

    <Section title="أعمال اليوم" description="أهم العناصر فقط في أعلى الصفحة؛ مركز أعمالك الكامل موجود أسفل نفس الشاشة.">
      {view.todayRows.length===0?<EmptyState title="لا يوجد شيء عاجل الآن" description="يمكنك إنشاء مهمة أو مراسلة من قسم أعمالي أسفل الصفحة."/>:<div style={{display:'grid',gap:8}}>{view.todayRows.map(task=><div key={task.id} style={{border:'1px solid var(--hair)',borderRadius:10,padding:'12px 14px'}}><div style={{display:'flex',gap:10,justifyContent:'space-between',alignItems:'flex-start'}}><div><strong>{task.title}</strong>{task.description&&<div className="hint" style={{marginTop:4}}>{task.description}</div>}</div><span>{STATUS_AR[task.status]||task.status}</span></div><div style={{display:'flex',gap:12,flexWrap:'wrap',marginTop:9,fontSize:12,opacity:.75}}><span>{TYPE_AR[task.task_type]||'عمل'}</span>{taskProject(task)&&<span>المشروع: {taskProject(task)}</span>}{task.due_at&&<span>الموعد: {dateKey(task.due_at)}</span>}<span>الإنجاز: {task.progress||0}%</span></div></div>)}</div>}
    </Section>

    <Section title="بواباتي ونطاق عملي" description="هذا انعكاس للصلاحيات الممنوحة فقط؛ اليوم لا يمنح صلاحية إضافية.">
      {view.projectsPortal&&<div className="msg ok" style={{marginBottom:12}}>لديك <strong>كامل بوابة المشاريع</strong> بكل وظائفها الحالية والمستقبلية. <Link href="/dashboard/workspace"><strong>فتح منصة الأعمال ←</strong></Link></div>}
    ...
    <div id="my-work" style={{scrollMarginTop:120,marginTop:22,borderTop:'2px solid var(--hair)',paddingTop:20}}>
      <MyWorkPage />
    </div>
  </ConstitutionPage>;
}
