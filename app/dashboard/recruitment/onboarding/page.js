...
export default function OnboardingQueue(){
  ...
  return <><div className="page-head"><div><h1>المباشرة والتهيئة</h1><p>من قبول العقد إلى مباشرة الموظف والتقييم خلال فترة التجربة</p></div><Link className="btn ghost" href="/dashboard/recruitment">التوظيف والمرشحون</Link></div>{err&&<div className="msg err" style={{marginBottom:12}}>{err}</div>}
  ...
  <div className="section" style={{marginTop:0,overflowX:'auto'}}><table><thead><tr><th>المرشح</th><th>الوظيفة</th><th>الحالة</th><th>المباشرة المتوقعة</th><th>المباشرة الفعلية</th><th>نهاية التجربة</th><th>الأساس النظامي للعمل</th><th>—</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td style={{fontWeight:600}}>{r.candidates?.full_name_ar||'—'}</td><td>{r.job_offers?.job_title_snapshot||'—'}</td><td>{S[r.status]||r.status}</td><td>{dateAr(r.expected_start_date)}</td><td>{dateAr(r.actual_start_date)}</td><td>{dateAr(r.probation_end_date)}</td><td>{r.work_authorization_basis||'—'}</td><td><Link className="btn ghost" style={{padding:'4px 10px',fontSize:12}} href={`/dashboard/recruitment/onboarding/${r.id}`}>فتح</Link></td></tr>)}{!rows.length&&<tr><td colSpan="8"><div className="empty">لا توجد ملفات تهيئة حتى الآن. تنشأ تلقائياً بعد قبول المرشح لمسودة العقد.</div></td></tr>}</tbody></table></div></>;
}
