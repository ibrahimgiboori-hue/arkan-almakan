...
export default function RecruitmentPage(){
  const [vacancies,setVacancies]=useState([]); const [apps,setApps]=useState([]); const [showNew,setShowNew]=useState(false);
  ...
  return <>
    <div className="page-head"><div><h1>التوظيف والمرشحون</h1><p>مسار واحد ظاهر: الشاغر ← الطلبات ← التقييم ← العرض ← التوظيف</p></div><button className="btn" onClick={()=>setShowNew(!showNew)}>+ شاغر جديد</button></div>
    ...
    <div className="section" style={{marginTop:0,overflowX:'auto'}}><header><h2>1. الشواغر</h2></header><table><thead><tr><th>رقم الشاغر</th><th>المسمى</th><th>الإدارة</th><th className="num">العدد</th><th>نطاق الراتب الداخلي</th><th>الحالة</th><th>فتح</th></tr></thead><tbody>{vacancies.map(v=><tr key={v.id}><td className="mono">{v.vacancy_no||'—'}</td><td style={{fontWeight:600}}>{v.title_ar}</td><td>{v.department||'—'}</td><td className="num">{v.headcount}</td><td className="mono">{v.salary_min!=null||v.salary_max!=null?`${v.salary_min!=null?money(v.salary_min):'—'} - ${v.salary_max!=null?money(v.salary_max):'—'}`:'يحدد لاحقاً'}</td><td>{VSTATUS[v.status]||v.status}</td><td><Link className="btn ghost" href={`/dashboard/recruitment/${v.id}`}>فتح</Link></td></tr>)}{!vacancies.length&&<tr><td colSpan="7"><div className="empty">لا توجد شواغر حتى الآن.</div></td></tr>}</tbody></table></div>

    <div className="section" style={{overflowX:'auto'}}><header><h2>2. طلبات المرشحين</h2></header><table><thead><tr><th>المرشح</th><th>الشاغر</th><th>الجنسية</th><th>انتهاء الهوية/الإقامة</th><th className="num">الاستبيان</th><th className="num">المقابلة</th><th className="num">النهائي</th><th>الحالة</th><th>فتح</th></tr></thead><tbody>{apps.map(a=><tr key={a.id}><td><strong>{a.candidates?.full_name_ar||'—'}</strong>{a.has_eliminating_issue&&<div style={{fontSize:12,color:'var(--warn)'}}>شرط أساسي يحتاج مراجعة</div>}</td><td>{a.job_vacancies?.title_ar||'—'}</td><td>{a.candidates?.nationality||'—'}</td><td className="mono">{dateAr(a.candidates?.id_expiry)}</td><td className="num">{a.questionnaire_score??'—'}</td><td className="num">{a.interview_score??'—'}</td><td className="num" style={{fontWeight:700}}>{a.final_score??'—'}</td><td>{ASTATUS[a.status]||a.status}</td><td><Link className="btn ghost" href={`/dashboard/recruitment/applications/${a.id}`}>فتح</Link></td></tr>)}{!apps.length&&<tr><td colSpan="9"><div className="empty">لم تصل طلبات ترشيح بعد.</div></td></tr>}</tbody></table></div>

    <div className="section"><header><h2>3. بنك المواهب</h2></header><div className="empty"><p>المرشحون الممتازون يبقون لمدة لا تتجاوز 90 يوماً ثم ينتقلون إلى الأرشيف المقيد.</p></div></div>
  </>;
}
