...
function money(value){return `${n(value).toLocaleString('ar-SA',{maximumFractionDigits:2})} ر.س`;}
...
async function loadPlanning(projectId){
  ...
  const rows=(itemsQ.data||[]).map(item=>{const d=durations.get(item.id)||{};const t=timing.get(item.id)||{};return [item.sort_order??'—',item.description_ar||'—',item.contract_qty??'—',item.unit||'—',date(t.forecast_start_date||d.first_day),date(t.forecast_end_date||d.last_day),d.days_spent??'—',d.total_output??0];});
...
async function loadCostControl(projectId){
  ...
  const rows=snapshots.map(r=>[date(r.snapshot_at),r.label||'لقطة مالية',`${Math.round(n(r.progress_pct))}%`,money(r.current_contract_value),money(r.earned_value),money(r.known_actual_cost),money(r.current_result),money(r.next_4w_inflow),money(r.next_4w_outflow)]);
...
async function loadChanges(projectId){
  ...
  return {summary:[{key:'count',label:'أوامر التغيير',value:rows.length,note:'المسجلة'},{key:'open',label:'غير معتمدة',value:rows.filter(r=>String(r.status||'').toLowerCase()!=='approved').length,note:'تحتاج متابعة'},{key:'days',label:'الأثر الزمني',value:`${rows.reduce((s,r)=>s+n(r.duration_days),0)} يوم`,note:'حسب الأوامر المسجلة'}],columns:['رقم التغيير','التاريخ','الوصف','السبب','الأثر الزمني','الحالة','اعتماد المالك'],rows:rows.map(r=>[r.co_number||'—',date(r.co_date),r.description||'—',r.reason||'—',`${n(r.duration_days)} يوم`,status(r.status),r.owner_ref||date(r.approved_at)]),note:'سجل المخاطر المستقل غير مضاف هنا لأنه يحتاج نموذج بيانات خاص؛ أما التغييرات فلها محرك موجود بالفعل.'};
}
...
async function loadCorrespondence(projectId){
  ...
  const rows=[...(siteQ.data||[]).map(r=>['مستند موقع',r.doc_kind||'—',r.title||'—',date(r.doc_date||r.created_at),'—']),...(docsQ.data||[]).map(r=>['مستند نظام',r.doc_number||'—',r.subject||'—',date(r.issued_at||r.sent_at||r.created_at),status(r.status)])];
