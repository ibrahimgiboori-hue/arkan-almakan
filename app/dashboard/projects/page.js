...
export default function Projects(){
  ...
  const [rows,setRows]=useState(null); const [fin,setFin]=useState({}); const [role,setRole]=useState(null);
  const [stage,setStage]=useState('all'); const [q,setQ]=useState(''); const [err,setErr]=useState(''); const [busy,setBusy]=useState(false);

  async function load(){
    setErr(''); const session=(await supabase.auth.getSession()).data.session;
    ...
    setRows(projectsQ.data||[]);setFin(map);setRole(userQ.error?null:(userQ.data?.role||null));
  }
  ...
  if(!rows)return <ConstitutionPage><EmptyState title="جارٍ تحميل المشاريع" description="يتم تحميل المشاريع والمؤشرات المالية الحالية."/></ConstitutionPage>;
  ...
  return <ConstitutionPage>
    <PageHeader eyebrow="PROJECTS" title="المشاريع" description="اختر المشروع الذي تريد العمل عليه. بعد الدخول إليه تصبح الشاشة لمساحة ذلك المشروع فقط." actions={canWrite?<Toolbar><button className="btn" onClick={createProject} disabled={busy}>{busy?'جارٍ الإنشاء…':'+ إضافة مشروع'}</button></Toolbar>:null}/>
    ...
    <Section title="سجل المشاريع" description={`${list.length} مشروع مطابق للعرض الحالي`}>
      {list.length===0?<EmptyState title="لا توجد مشاريع مطابقة" description="غيّر البحث أو الفلتر الحالي، أو أضف مشروعًا جديدًا."/>:<div className={styles.projectGrid} aria-label="بطاقات المشاريع">
        {list.map((project)=>{
          ...
          return <button key={project.id} className={styles.projectCard} onClick={()=>router.push(`/dashboard/projects/${project.id}`)}>
            ...
            <div className={styles.cardNumbers}><div><span>قيمة العقد</span><strong>{money(project.contract_value||0)}</strong></div><div><span>غير محصل</span><strong className={pending>0?styles.warn:''}>{money(pending)}</strong></div><div><span>الربح الحالي</span><strong className={profit<0?styles.danger:''}>{money(profit)}</strong></div></div>
