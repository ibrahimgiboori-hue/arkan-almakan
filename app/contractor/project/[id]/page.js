...
const STATUS=[['absent','غياب'],['half','نصف يوم'],['full','حضور كامل']];
...
export default function ContractorProjectTimesheet(){
  ...
  return <>
    ...
    <section className="portal-section"><header><h2>عمالة المقاول</h2><span>{rows.length} فردًا</span></header>
      {rows.length?<div className="portal-roster">{rows.map((row,index)=><article key={row.laborer_id}><div className="portal-worker"><span>{index+1}</span><div><b>{row.full_name}</b><small>{row.trade||laborClassLabel(row.labor_class)}</small>{row.last_edited_by_name&&<em>عُدّل بواسطة {row.last_edited_by_name} · {new Date(row.last_edited_at).toLocaleString('ar-SA-u-ca-gregory')}</em>}</div><button onClick={()=>setEditing(row)}>بيانات العامل</button></div><div className="portal-attendance"><label>الحالة<select value={draft[row.laborer_id]?.status||'absent'} onChange={e=>change(row.laborer_id,'status',e.target.value)}>{STATUS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>ملاحظة داخلية<input value={draft[row.laborer_id]?.notes||''} onChange={e=>change(row.laborer_id,'notes',e.target.value)} placeholder="اختياري"/></label></div></article>)}</div>:<div className="portal-empty">لا توجد عمالة مسندة لهذا المشروع في التاريخ المختار.</div>}
