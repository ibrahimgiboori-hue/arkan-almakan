...
export default function BlankContractorTimesheet(){
  ...
  useEffect(()=>{(async()=>{
    let query=await supabase.from('app_settings')
      .select('company_name_ar,letterhead_image_path,header_image_path,footer_image_path,watermark_image_path,header_height_mm,footer_height_mm')
      .eq('id',1).maybeSingle();
  ...
  return <>
    ...
    <ConstitutionPrintFrame documentKey="timesheet_blank" cfg={cfg}>
      <div className="ts-page ts-blank-page">
        <div className="ts-doc-title" data-print-keep-with-next="true"><h1>نموذج تايم شيت أسبوعي</h1><span/></div>
        ...
        <div className="ts-paper-instruction" data-print-keep-with-next="true">يكتب اسم العامل وصفته، ثم تسجل حالة الحضور في كل يوم: ✓ يوم كامل أو ½ نصف يوم، ويترك الغائب بلا علامة.</div>
        <table className="ts-table ts-blank-table" data-print-flow={PRINT_FLOW_KIND.REPEATABLE_TABLE}>
          ...
          <tfoot><tr data-print-row-role="total" data-print-row-atomic="true"><th colSpan={3}>إجمالي اليوميات لكل يوم</th>{WEEK_DAYS.map(day=><th key={day}>........</th>)}<th/></tr></tfoot>
        </table>
