...
const BLUEPRINT_PLACEHOLDER_PATTERNS = [
  /LETTERHEAD/i,
  /FOOTER\s*\/\s*LETTERHEAD\s*RESERVED/i,
  /LETTERHEAD\s*RESERVED/i,
  /مساحة\s+الهوية\s+المحمية/,
  /الهوية\s+المحمية/,
];
...
function isBlueprintPlaceholder(value){const text=String(value||'').trim();return Boolean(text)&&BLUEPRINT_PLACEHOLDER_PATTERNS.some((pattern)=>pattern.test(text))}
function printableText(value){return isBlueprintPlaceholder(value)?'':String(value||'').trim()}
function printableLines(value){return String(value||'').split('\n').map((line)=>line.trim()).filter((line)=>line&&!isBlueprintPlaceholder(line))}
function hasTermContent(term){return Boolean(printableText(term?.title)||printableText(term?.body))}
function cleanTerm(term){return {...term,title:printableText(term?.title),body:printableText(term?.body)}}
...
export default function QuotePrint(){
  ...
  const quoteSpecificTerms=printableLines(q.terms_text);
  const sourceTerms=(Array.isArray(q.terms_structured)?q.terms_structured:[]).map(cleanTerm).filter(hasTermContent);
  ...
  const lineDesc=line=>printableText(isEn?(line.description_en||line.description_ar||''):(line.description_ar||''));
  ...
  const title=printableText(q.title_override)||(q.doc_kind==='boq'?tr('جدول كميات','BILL OF QUANTITIES (BOQ)'):tr('عرض سعر','QUOTATION'));
  const introText=printableText(q.intro_text);
  const closingText=printableText(q.closing_text);
  ...
  const metaCells=[];
  if(q.show_client!==false)metaCells.push(<div className="mcell" key="client"><span className="mk">{tr('العميل','Client')}</span><span className="mv">{printableText(q.client_name)}</span>{printableText(q.client_contact)&&<span className="ms">{printableText(q.client_contact)}</span>}</div>);
  if(q.show_project!==false)metaCells.push(<div className="mcell" key="project"><span className="mk">{tr('المرجع وتفاصيل المشروع','Project / Reference')}</span><span className="mv">{printableText(q.project_ref)||'—'}</span>{printableText(q.site_location)&&<span className="ms">{printableText(q.site_location)}</span>}</div>);
  ...
  return <div dir={dir} className="print-doc-quotation-route">
    ...
    >
      <div className="quote-document-flow" dir={dir}>
        ...
        {q.show_intro&&introText?<p className="q-intro">{introText}</p>:null}
        ...
        {q.show_terms&&termItems.length?<>
          ...
          {termItems.map(term=><section className="q-term-flow" key={term.id}>
            {term.title?<h3><span className="term-no">{term.number}.</span>{term.title}</h3>:null}
            {term.body?<p>{!term.title?<><span className="term-no">{term.number}.</span>{' '}</>:null}{term.body}</p>:null}
          </section>)}
        </>:null}
        {q.show_closing&&closingText?<p className="q-closing">{closingText}</p>:null}
