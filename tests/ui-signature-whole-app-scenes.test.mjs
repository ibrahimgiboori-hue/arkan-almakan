...
const routes = [
  ...
  '/dashboard/employees','/dashboard/attendance','/dashboard/recruitment','/dashboard/recruitment/offers',
  '/dashboard/recruitment/contracts','/dashboard/recruitment/onboarding','/dashboard/leaves','/dashboard/leave-history-import',
  '/dashboard/advances','/dashboard/operating-budget','/dashboard/expenses',
...
test('visible app routes receive contextual Signature scenes', () => {
  for (const route of routes) assert.ok(runtime.includes(route), `${route} must map to a scene`);
});

test('whole-app runtime yields project-detail pages to project scene runtime', () => {
...
test('whole-app scene layer is screen-only with strict readable foreground surfaces', () => {
  ...
  assert.match(skin, /--sig-fallback/);
});
